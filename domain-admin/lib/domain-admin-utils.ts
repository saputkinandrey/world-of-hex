...
const storageKey = 'wohex.domain-admin.config.v1';
const filePathStorageKey = 'wohex.domain-admin.config-file-path.v1';
...
const cloneConfig = (config: DomainAdminConfig): DomainAdminConfig => {
  return JSON.parse(JSON.stringify(config)) as DomainAdminConfig;
};
...
const normalizeConfig = (config: DomainAdminConfig): DomainAdminConfig => {
  const legacyConfig = config as DomainAdminConfig & {
  ...
  const normalized = cloneConfig(config) as DomainAdminConfig & {
    familyRules?: unknown;
    familySeed?: unknown;
    familyScale?: unknown;
    areaTypes?: unknown;
...
const getGood = (config: DomainAdminConfig, goodId: string): Good => {
  return (
...
const getGoodProfile = (config: DomainAdminConfig, goodType: string): GoodProfile => {
  return (
...
const getGoodLevelName = (config: DomainAdminConfig, level: number): string => {
  return config.goodLevels.find((goodLevel) => goodLevel.level === level)?.name ?? 'Missing level';
};

const getEdibleGoods = (config: DomainAdminConfig): Good[] => {
...
const getSelectedRationTemplate = (config: DomainAdminConfig): RationTemplate => {
  return (
...
const getConsumerNeedProfile = (config: DomainAdminConfig, consumerId: string): ConsumerNeedProfile => {
...
const getRationCalculatorItems = (config: DomainAdminConfig, items: RationItem[]): RationCalculatorItem[] => {
...
const calculateRation = (config: DomainAdminConfig): RationResult => {
...
const solveRationItems = (config: DomainAdminConfig, template: RationTemplate): RationItem[] => {
  const target = getConsumerNeedProfile(config, template.consumerId);
