...
const stepsById = new Map(
    config.productionChains.flatMap((chain) =>
        chain.recipes.map((step) => [step.id, step]),
    ),
);
...
for (const chain of config.productionChains) {
    for (const step of chain.recipes) {
        assert.equal(
            typeof step.durationMinutes,
            "number",
            `${chain.id}/${step.id} should define durationMinutes`,
        );
        assert.ok(
            step.durationMinutes >= 0,
            `${chain.id}/${step.id} durationMinutes should be non-negative`,
        );
        for (const edge of [...step.inputs, ...step.outputs]) {
            assert.ok(
                Number.isInteger(edge.quantity),
                `${chain.id}/${step.id}/${edge.goodId} quantity should be an integer unit count`,
            );
        }
    }
}

for (const chain of config.productionChains.filter((candidate) =>
    recipeChainIds.has(candidate.id),
)) {
    for (const step of chain.recipes) {
        assert.ok(
            step.durationMinutes > 0,
            `${chain.id}/${step.id} should have a realistic non-zero duration`,
        );
    }
}
...
const expectedQuantities = [
    ...
    [
        "production-step-food-cabbage-food-radish-spice-chili-flakes-food-kimchi",
        "food-salt",
        29,
    ],
    [
...
for (const [stepId, goodId, expectedQuantity] of expectedQuantities) {
    ...
    assert.equal(
        input.quantity,
        expectedQuantity,
        `${stepId}/${goodId} should have realistic recipe quantity`,
    );
}

const onionSoup = stepsById.get("production-step-dish-yellow-onion-soup");
assert.ok(
    onionSoup.inputs.some((input) => input.goodId === "food-sourdough-bread"),
    "onion soup should include bread",
);
assert.ok(
    onionSoup.inputs.some((input) => input.goodId === "food-aged-raw-cheese"),
    "onion soup should include cheese",
);

const cipollini = stepsById.get("production-step-dish-cipollini-agrodolce");
assert.ok(
    cipollini.inputs.some(
        (input) => input.goodId === "food-apple-cider-vinegar",
    ),
    "agrodolce should include vinegar",
);
