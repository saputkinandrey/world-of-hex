...
const expectedDishes = ["dish-stuffed-bell-pepper", "dish-rajas-poblano", "dish-padron-tapas"];

for (const goodId of [...expectedHarvestSources.keys(), ...expectedProcessedGoods, ...expectedDishes]) {
  assert.ok(goodsById.has(goodId), `${goodId} should be imported from pepper source coverage`);
}
...
for (const [goodId, sourceGoodId] of expectedHarvestSources) {
  ...
  assert.equal(good.level, 1, `${goodId} should be a harvested primary pepper`);
  assert.equal(good.goodType, "Овощи", `${goodId} should be modeled as a vegetable`);
  ...
  assert.equal(sourceGood.goodType, "Растения", `${sourceGoodId} should be a plant source`);
  assert.ok(sourceGood.biomeIds.length > 0, `${sourceGoodId} should be placed in at least one biome`);
  assert.ok(
    vegetableHarvesting.steps.some(
      (step) =>
...
for (const goodId of expectedProcessedGoods) {
  assert.ok(
    pepperProcessing.steps.some((step) => step.outputs.some((output) => output.goodId === goodId)),
...
assert.equal(pepperDishes.steps.length, 3, "pepper dishes should cover source-supported dishes possible with current goods");

const legumeDishes = chainsById.get("production-chain-legume-dishes");
assert.ok(legumeDishes, "legume dishes chain should exist");

const redBeanChili = legumeDishes.steps.find((step) => step.id === "production-step-dish-red-bean-chili");
assert.ok(redBeanChili.inputs.some((input) => input.goodId === "food-jalapeno"), "red bean chili should include jalapeno");
assert.ok(redBeanChili.inputs.some((input) => input.goodId === "spice-chipotle-powder"), "red bean chili should include chipotle powder");

const blackBeansRice = legumeDishes.steps.find((step) => step.id === "production-step-dish-black-beans-rice");
assert.ok(blackBeansRice.inputs.some((input) => input.goodId === "food-serrano-pepper"), "black beans with rice should include serrano");

const refriedPinto = legumeDishes.steps.find((step) => step.id === "production-step-dish-refried-pinto-beans");
assert.ok(refriedPinto.inputs.some((input) => input.goodId === "food-jalapeno"), "refried pinto beans should include jalapeno");
