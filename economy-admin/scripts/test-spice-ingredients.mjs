...
for (const goodId of expectedSpiceGoods) {
  ...
  if (good.goodType !== 'Специи' || good.level < 1 || good.biomeIds.length > 0) {
...
for (const goodId of expectedSourceGoods) {
  ...
  if (good.goodType !== 'Растения' || good.level !== 0 || good.biomeIds.length === 0) {
    throw new Error(`Invalid spice source schema: ${goodId}`);
  }
}

const harvestingChain = config.productionChains.find((chain) => chain.id === 'production-chain-spice-harvesting');
const processingChain = config.productionChains.find((chain) => chain.id === 'production-chain-spice-processing');
...
const msgStep = processingChain.steps.find((step) => step.id === 'production-step-research-seaweed-spice-msg');
if (!msgStep || !msgStep.inputs.some((input) => input.goodId === 'research-seaweed')) {
  throw new Error('MSG must be modeled as concentrated seasoning from existing dried kelp.');
}

console.log('Spice article ingredients are modeled as goods and production chains.');
