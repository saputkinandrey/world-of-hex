...
const liquidProfile = config.goodProfiles.find(
    (profile) => profile.goodType === "Жидкости",
);

if (
    !liquidProfile ||
    liquidProfile.edible !== true ||
    liquidProfile.waterRating !== 1 ||
    liquidProfile.weightPerUnitLbs !== 0
) {
    throw new Error(
        "Жидкости profile must be available to ration solving as 100 ml water units outside food-weight satiety.",
    );
}
...
for (const expectedLiquid of expectedLiquids) {
    ...
    if (
        liquid.goodType !== "Жидкости" ||
        liquid.level !== expectedLiquid.level
    ) {
...
if (water.biomeIds.length !== config.biomes.length) {
    throw new Error(
        "Water must be available as a level 0 resource in every biome.",
    );
}

const waterChain = config.productionChains.find(
    (chain) => chain.id === "production-chain-water-processing",
);
if (!waterChain) {
...
const expectedOutputs = new Set(
    expectedLiquids
        .filter((liquid) => liquid.id !== "resource-water")
        .map((liquid) => liquid.id),
);
const actualOutputs = new Set(
    waterChain.recipes.flatMap((step) =>
...
const missingReferences = config.productionChains
    .flatMap((productionChain) =>
        productionChain.recipes.flatMap((step) => [
