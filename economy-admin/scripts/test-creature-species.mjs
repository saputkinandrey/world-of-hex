...
const economyAdminDirectory = path.resolve(
    path.dirname(fileURLToPath(import.meta.url)),
    "..",
);
const workspaceDirectory = path.resolve(economyAdminDirectory, "..");
...
const contextSource = readWorkspaceFile(
    "economy-admin/context/EconomyAdminContext.tsx",
);
const appSource = readWorkspaceFile(
    "economy-admin/components/EconomyAdminApp.tsx",
);
const screenSource = readWorkspaceFile(
    "economy-admin/components/screens/CreatureSpeciesScreen.tsx",
);
const creatureProfilesUtilsSource = readWorkspaceFile(
    "economy-admin/lib/creature-profiles-utils.ts",
);
...
const routeSource = readWorkspaceFile(
    "economy-admin/app/api/creature-profiles/route.ts",
);
...
assert.ok(
    readWorkspaceFile("economy-admin/lib/creature-profiles-utils.ts").includes(
...
assert.ok(
    bodySizeSource.includes("BODY_WEIGHT_SM0_ST10_AVERAGE_BUILD_LB") &&
        bodySizeSource.includes("BODY_WEIGHT_SR_LINEAR_MEASUREMENT_INCHES") &&
        bodySizeSource.includes("roundImperialMassLb") &&
        bodySizeSource.includes("Math.pow(linearScale, 3)") &&
        ...
        bodySizeSource.includes("estimateStomachCapacityLb") &&
    ...
    "Creature species editor must expose the SR-table square-cube body and stomach estimates.",
);
assert.ok(
    actorEntitySource.includes("estimateStomachCapacityLb(sm).capacityLb") &&
        actorEntitySource.includes("number | undefined") &&
        actorEntitySource.includes(
            "Cannot estimate stomach capacity without explicit size modifier morph.",
        ) &&
        !actorEntitySource.includes("Math.pow(2, sm)") &&
        actorEntitySource.includes("profileSizeModifier"),
    "Actor stomach capacity must use explicit SM and fail clearly when SM is missing.",
);
assert.ok(
    actorFactorySource.includes("estimateDailyFoodMassNeedLb") &&
        actorFactorySource.includes("creatureProfile: template"),
    "Actor factory must derive daily food mass from SM and attach the creature profile.",
);
assert.ok(
