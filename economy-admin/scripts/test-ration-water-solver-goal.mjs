...
const goodsEntityPath = path.resolve(scriptDirectory, '..', 'entities', 'goods.ts');
const consumersEntityPath = path.resolve(scriptDirectory, '..', 'entities', 'consumers.ts');
...
const missingSnippets = [
  ...requiredEntitySnippets.filter((snippet) => !goodsEntitySource.includes(snippet) && !consumersEntitySource.includes(snippet)),
  ...requiredPageSnippets.filter((snippet) => !utilsSource.includes(snippet)),
  ...requiredCalculatorSnippets.filter((snippet) => !calculatorSource.includes(snippet)),
];
if (missingSnippets.length > 0) {
  throw new Error(`Ration solver must include water as an optimization goal. Missing: ${missingSnippets.join(', ')}`);
}
