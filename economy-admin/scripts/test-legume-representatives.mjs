...
const legumeNames = new Set(config.goods.filter((good) => good.goodType === 'Бобовые').map((good) => good.name));
const missingLegumes = expectedLegumeNames.filter((name) => !legumeNames.has(name));
if (missingLegumes.length > 0) {
  throw new Error(`Missing article legume representatives: ${missingLegumes.join(', ')}`);
}

if (legumeNames.has('Фасоль')) {
  throw new Error('Generic Фасоль must be replaced with concrete bean representatives.');
}
...
if (missingProductionRefs.length > 0) {
  throw new Error(`Production chains contain missing good refs: ${missingProductionRefs.join(', ')}`);
}
