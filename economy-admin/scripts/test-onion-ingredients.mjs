...
for (const goodId of expectedGoods) {
  assert.ok(goodsById.has(goodId), `${goodId} should be imported from onion source coverage`);
}

const levelOneOnions = expectedGoods.filter((goodId) => goodId.startsWith("food-") && !goodId.includes("caramelized") && !goodId.includes("fried"));
...
for (const goodId of levelOneOnions) {
  assert.equal(goodsById.get(goodId).level, 1, `${goodId} should be a harvested primary good`);
  assert.equal(goodsById.get(goodId).goodType, "Овощи", `${goodId} should be modeled as a vegetable`);
}
...
assert.equal(onionDishes.steps.length, 6, "onion dishes chain should cover dishes mentioned by the onion source");

const legumeDishes = chainsById.get("production-chain-legume-dishes");
assert.ok(legumeDishes, "legume dishes chain should exist");

const genericOnionLegumeInputs = legumeDishes.steps.flatMap((step) =>
  step.inputs.filter((input) => input.goodId === "research-onion").map((input) => `${step.id}/${input.id}`),
);
assert.deepEqual(genericOnionLegumeInputs, [], "legume dishes should use concrete onion variants instead of generic onion");
