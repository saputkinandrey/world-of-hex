...
const useEconomyAdminState = () => {
                ...
                try {
                    if (!cancelled) {
                        const loadedConfig = normalizeConfig(
                            JSON.parse(raw) as EconomyAdminConfig,
                        );
                        setConfig(loadedConfig);
                        setIsConfigTextDirty(false);
                    }
    ...
    const patchConfig = (patch: Partial<EconomyAdminConfig>) =>
    ...
    const importConfigText = () => {
        try {
            syncConfigState(
                normalizeConfig(JSON.parse(configText) as EconomyAdminConfig),
            );
            setIsConfigTextDirty(false);
            setConfigError(null);
...
const EconomyAdminContext = createContext<EconomyAdminContextValue | null>(
    null,
);
...
export const EconomyAdminProvider = ({ children }: ConfigProviderProps) => {
    ...
    return (
        <EconomyAdminContext.Provider value={value}>
            {children}
        </EconomyAdminContext.Provider>
    );
};
...
export const useEconomyAdmin = (): EconomyAdminContextValue => {
    ...
    if (!value) {
        throw new Error(
            "useEconomyAdmin must be used inside EconomyAdminProvider",
        );
    }
    return value;
};
