...
const EconomyAdminContent = () => {
  const { tab, setTab, foodNavigationValue, economyNavigationValue } = useEconomyAdmin();

  return (
    <Box className="admin-shell">
      <AppBar position="sticky" elevation={0} sx={{ bgcolor: 'rgba(7, 24, 21, 0.92)', borderBottom: '1px solid var(--line)' }}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} alignItems={{ xs: 'stretch', sm: 'center' }}>
              <FormControl size="small" sx={{ minWidth: { xs: '100%', sm: 220 } }}>
                ...
                <Select
                  ...
                  displayEmpty
                  onChange={(event) => setTab(event.target.value as EconomyAdminTab)}
                >
              ...
              <Button
                variant={tab === 'config' ? 'contained' : 'outlined'}
