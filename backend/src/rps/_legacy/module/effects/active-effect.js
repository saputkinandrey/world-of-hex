...
export default class GurpsActiveEffect extends ActiveEffect {
  ...
  static _preCreate(_effect, data, _options, _userId) {
    ...
    if (data.duration && !data.duration.combat && game.combat)
  ...
  static async _apply(actor, change, _options, _user) {
    if (change.key === 'system.conditions.maneuver')
      actor.replaceManeuver(change.value);
    else if (change.key === 'system.conditions.posture')
      actor.replacePosture(change);
        ...
        for (const effect of token.actor.effects) {
          if (await effect.isExpired()) {
            ...
            ui.notifications.info(
              `${i18n('GURPS.effectExpired', 'Effect has expired: ')} '[${i18n(effect.label)}]'`,
            );
          }
        }
      }
  ...
  static async clearEffectsOnSelectedToken() {
    ...
    for (let i = 0; i < effect.length; i++) {
      ...
      console.debug(
        `Clear Effect: condition: [${condition}] status: [${status}] effect_id: [${effect_id}]`,
      );
      if (status === false) {
  ...
  chat(actor, value) {
    ...
    for (const key in value.args) {
      ...
      if (foundry.utils.getType(val) === 'string' && val.startsWith('@')) {
        ...
        foundry.utils.getType(val) === 'string' &&
        val.startsWith('!')
      ) {
