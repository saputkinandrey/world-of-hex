...
export class StatusEffect {
  ...
  static useActiveEffects() {
    return game.settings.get(
      SYSTEM_NAME,
      StatusEffect.SETTING_USE_ACTIVE_EFFECTS,
    );
  }
  ...
  _registerSetting() {
    game.settings.register(
      ...
      StatusEffect.SETTING_USE_ACTIVE_EFFECTS,
      {
        ...
        default: false,
        onChange: (value) =>
          console.log(`${StatusEffect.SETTING_USE_ACTIVE_EFFECTS} : ${value}`),
      },
    );
  }
  ...
  getAllPostures() {
    let postures = Object.keys(this._statusEffects).reduce(
      (accumulator, key) => {
        if (
          foundry.utils.getProperty(
            this._statusEffects[key],
            'flags.gurps.effect.type',
          ) == 'posture'
        )
