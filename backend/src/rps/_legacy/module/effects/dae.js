...
async function _setFlag(data) {
  if (!data.actorUuid)
    return await game.actors
      .get(data.actorId)
      ?.setFlag('dae', data.flagId, data.value);
  else
    return await DAEfromActorUuid(data.actorUuid)?.setFlag(
      'dae',
      data.flagId,
      data.value,
    );
...
async function _renameToken(data) {
  return await canvas.tokens.placeables
    .find((t) => t.id === data.tokenData._id)
...
export async function createToken(tokenData, x, y) {
  ...
  return socketlibSocket.execuateAsGM('createToken', {
    userId: game.user.id,
    targetSceneId,
    tokenData,
    x,
...
export let teleport = async (tokenDocument, targetScene, xpos, ypos) => {
  ...
  if (targetScene.name === canvas.scene.name) {
    //@ts-ignore
    CanvasAnimation.terminateAnimation(
      `Token.${tokenDocument.id}.animateMovement`,
    );
  ...
  Hooks.once('canvasReady', async () => {
    ...
    await socketlibSocket.executeAsGM('deleteToken', {
      userId: game.user.id,
      tokenUuid: tokenDocument.uuid,
    });
