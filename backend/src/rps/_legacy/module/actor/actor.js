...
export class GurpsActor extends Actor {
  ...
  getOwners() {
    return game.users?.contents.filter(
      (u) => this.getUserLevel(u) >= CONST.DOCUMENT_PERMISSION_LEVELS.OWNER,
    );
  }
  ...
  prepareDerivedData() {
    ...
    if (this.system.encumbrance) {
      ...
      if (!move) {
        let currentMove =
          this.system.encumbrance['00000'].move ?? this.system.basicmove.value;
        let value = {
          mode: MoveModes.Ground,
          basic: currentMove,
          default: true,
        };
  ...
  async postImport() {
    ...
    // Convoluted code to add Items (and features) into the armor-builder list
    // @ts-ignore
    ...
    for (const item of good) await this.addItemData(item); // re-add the item armor-builder and features
  ...
  calculateDerivedValues() {
    ...
    let maneuver = this.effects.contents.find(
      (it) => it.flags?.core?.statusId === 'maneuver',
    );
    this.system.conditions.maneuver = !!maneuver
      ? maneuver.flags.gurps.name
      : 'undefined';
  ...
  _initializeStartingValues() {
    ...
    if (!!data.equipment && !data.equipment.carried)
      data.equipment.carried = {}; // data protection
        ...
        for (let bonus of bonuses) {
          ...
          if (!!link.action) {
            ...
            recurselist(data.melee, (e, k, d) => {
              ...
              if (link.action.type == 'attack' && !!link.action.isMelee) {
                if (
                  e.name.match(makeRegexPatternFrom(link.action.name, false))
                ) {
                  e.level += pi(link.action.mod);
                  if (!isNaN(parseInt(e.parry))) {
                    // handles '11f'
            ...
            recurselist(data.skills, (e, k, d) => {
              ...
              if (link.action.type == 'attribute') {
                ...
                if (
                  e.relativelevel?.toUpperCase().startsWith(link.action.attrkey)
                )
                  e.level += pi(link.action.mod);
              }
              if (
                link.action.type == 'skill-spell' &&
                !link.action.isSpellOnly
              ) {
                if (e.name.match(makeRegexPatternFrom(link.action.name, false)))
                  e.level += pi(link.action.mod);
              }
          ...
          if (!!m) {
            ...
            if (!!m[2]) {
              ...
              locpatterns = locs.map(
                (l) => new RegExp(makeRegexPatternFrom(l), 'i'),
              );
            }
            recurselist(data.hitlocations, (e, _k, _d) => {
              ...
              if (
                !locpatterns ||
                locpatterns.find((p) => !!e.where && e.where.match(p)) != null
              ) {
                let dr = e.dr ?? '';
                dr += '';
  ...
  _findEqtkeyForId(key, id) {
    ...
    recurselist(data.equipment.carried, (e, k, d) => {
      if (e[key] == id) eqtkey = 'system.armor-builder.carried.' + k;
    });
    if (!eqtkey)
      recurselist(data.equipment.other, (e, k, d) => {
        if (e[key] == id) eqtkey = 'system.armor-builder.other.' + k;
      });
    return eqtkey;
  }
  ...
  _calculateWeights() {
    ...
    let eqtsummary = {
      ...
      eqtlbs: this._sumeqt(
        ...
        game.settings.get(
          ...
          settings.SETTING_CHECK_EQUIPPED,
        ),
      ),
    ...
    if (
      game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_AUTOMATIC_ENCUMBRANCE,
      )
    )
  ...
  _calculateEncumbranceIssues() {
    ...
    if (!!encs) {
      ...
      let effectiveDodge = isNaN(parseInt(level0.dodge))
        ? '–'
        : parseInt(level0.dodge) + data.currentdodge;
      let effectiveSprint = this._getSprintMove();

      if (isReeling) {
        effectiveMove = Math.ceil(effectiveMove / 2);
        effectiveDodge = isNaN(effectiveDodge)
          ? '–'
          : Math.ceil(effectiveDodge / 2);
      ...
      for (let enckey in encs) {
        ...
        enc.currentdodge = isNaN(effectiveDodge)
          ? '–'
          : Math.max(1, effectiveDodge - parseInt(enc.level));
        enc.currentsprint = Math.max(
          1,
          Math.floor(effectiveSprint * threshold),
        );
    ...
    if (!data.currentmove)
      data.currentmove = parseInt(data.basicmove.value.toString());
    if (!data.currentdodge && data.dodge.value)
      data.currentdodge = parseInt(data.dodge.value.toString());
    if (!data.currentflight)
      data.currentflight = parseFloat(data.basicspeed.value.toString()) * 2;
  }
  ...
  _getCurrentMove(move, threshold) {
    ...
    let updateMove =
      game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_MANEUVER_UPDATES_MOVE,
      ) && inCombat;
    ...
    if (threshold == 1.0)
      this.system.conditions.move =
        maneuver.move < posture.move ? maneuver.text : posture.text;
    return updateMove
  ...
  _getMoveAdjustedForManeuver(move, threshold) {
    ...
    if (foundry.utils.getProperty(this, PROPERTY_MOVEOVERRIDE_MANEUVER)) {
      let value = foundry.utils.getProperty(
        this,
        PROPERTY_MOVEOVERRIDE_MANEUVER,
      );
    ...
    switch (value) {
      ...
      case MOVE_ONE:
        return {
          ...
          text: i18n_f(
            'GURPS.moveConstant',
            { value: 1, unit: 'yard', reason: reason },
            '1 {unit}/second',
          ),
        };
  ...
  _getMoveAdjustedForPosture(move, threshold) {
    ...
    if (foundry.utils.getProperty(this, PROPERTY_MOVEOVERRIDE_POSTURE)) {
      ...
      let reason = i18n(
        GURPS.StatusEffect.lookup(this.system.conditions.posture).label,
      );
      adjustment = this._adjustMove(move, threshold, value, reason);
    }
  ...
  _calculateRangedRanges() {
    if (
      !game.settings.get(settings.SYSTEM_NAME, settings.SETTING_CONVERT_RANGED)
    )
      return;
  ...
  async update(data, context) {
    if (
      game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_AUTOMATIC_ONETHIRD,
      )
    ) {
      if (data.hasOwnProperty('system.HP.value')) {
        ...
        if (!!this.system.conditions.reeling != flag) {
          ...
          if (
            game.settings.get(
              settings.SYSTEM_NAME,
              settings.SETTING_SHOW_CHAT_FOR_REELING_TIRED,
            )
          ) {
  ...
  async replacePosture(changeData) {
    ...
    if (tokens)
      for (const t of tokens) {
        let id =
          changeData === GURPS.StatusEffectStanding
            ? this.system.conditions.posture
            : changeData;
  ...
  get displayname() {
    let n = this.name;
    if (!!this.token && this.token.name != n)
      n = this.token.name + '(' + n + ')';
    return n;
  }
  ...
  async accumulateDamageRoll(action) {
    ...
    await this.internalUpdate({
      'system.conditions.damageAccumulators': accumulators,
    });
  ...
  async incrementDamageAccumulator(index) {
    ...
    await this.internalUpdate({
      'system.conditions.damageAccumulators': this.damageAccumulators,
    });
    GURPS.ModifierBucket.render();
  }
  ...
  async decrementDamageAccumulator(index) {
    this.damageAccumulators[index].count--;
    if (this.damageAccumulators[index].count < 1)
  ...
  async _openImportDialog() {
    if (
      game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_USE_BROWSER_IMPORTER,
      )
    )
      ...
      new Dialog(
        {
          ...
          content: await renderTemplate(
            'systems/gurps/templates/import-gcs-v1-data.html',
            {
              name: '"' + this.name + '"',
            },
          ),
  ...
  async importAttributesFromGCSv2(atts, eqp, calc) {
    ...
    if (!!data.lastImport && (data.HP.value != hp || data.FP.value != fp)) {
      let option = game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_IMPORT_HP_FP,
      );
      if (option == 0) {
        saveCurrent = true;
      }
    ...
    data.basicmove.points =
      atts.find((e) => e.attr_id === 'basic_move')?.calc?.points || 0;
    let bs = atts.find((e) => e.attr_id === 'basic_speed')?.calc?.value || 0;
    data.basicspeed.value = bs.toString();
    data.basicspeed.points =
      atts.find((e) => e.attr_id === 'basic_speed')?.calc?.points || 0;

    data.thrust = calc?.thrust;
    data.swing = calc?.swing;
    data.currentmove = data.basicmove.value;
    data.frightcheck =
      atts.find((e) => e.attr_id === 'fright_check')?.calc?.value || 0;

    data.hearing = atts.find((e) => e.attr_id === 'hearing')?.calc?.value || 0;
    data.tastesmell =
      atts.find((e) => e.attr_id === 'taste_smell')?.calc?.value || 0;
  ...
  calcTotalCarried(eqp) {
    ...
    for (let i of eqp) {
      ...
      w +=
        ...
        (i.type == 'equipment_container' ? 1 : i.quantity || 0);
  ...
  getPortraitPath() {
    if (
      game.settings.get(settings.SYSTEM_NAME, settings.SETTING_PORTRAIT_PATH) ==
      'global'
    )
  ...
  importAd(i, p) {
    ...
    if (i.cr != null) {
      a.notes =
        '[' +
        game.i18n.localize('GURPS.CR' + i.cr.toString()) +
        ': ' +
        a.name +
        ']';
    }
  ...
  importSk(i, p) {
    let name =
      i.name +
        (!!i.tech_level ? `/TL${i.tech_level}` : '') +
        (!!i.specialization ? ` (${i.specialization})` : '') || 'Skill';
  ...
  importEq(i, p, carried) {
    ...
    e.cost =
      (
        parseFloat(i.calc?.extended_value) /
        (i.type == 'equipment_container' ? 1 : i.quantity || 1)
      ).toString() || '';
    e.carried = carried;
    ...
    if (i.modifiers?.length) {
      for (let j of i.modifiers)
        if (!j.disabled)
          e.notes += `${!!e.notes ? '; ' : ''}${j.name}${!!j.notes ? ' (' + j.notes + ')' : ''}`;
    }
    if (!!e.note) e.notes += (!!e.notes ? '\n' : '') + e.note;
    e.weight =
      (
        parseFloat(i.calc?.extended_weight) /
        (i.type == 'equipment_container' ? 1 : i.quantity || 1)
      ).toString() || '0';
    e.pageRef(i.reference || '');
    let old = this._findElementIn('armor-builder.carried', e.uuid);
    if (!old) old = this._findElementIn('armor-builder.other', e.uuid);
  ...
  async importProtectionFromGCSv2(hls) {
    ...
    let vitals = locations.filter(
      (value) => value.where === HitLocations.HitLocation.VITALS,
    );
    if (vitals.length === 0) {
      let hl = new HitLocations.HitLocation(HitLocations.HitLocation.VITALS);
      hl.penalty =
        HitLocations.hitlocationRolls[HitLocations.HitLocation.VITALS].penalty;
      hl.roll =
        HitLocations.hitlocationRolls[HitLocations.HitLocation.VITALS].roll;
      hl.import = '0';
      locations.push(hl);
    }
    ...
    if (!!table) {
      Object.values(locations).forEach((it) => {
        let [lbl, entry] = HitLocations.HitLocation.findTableEntry(
          table,
          it.where,
        );
        if (!!entry) {
          ...
          if (
            !it.roll ||
            it.roll.length === 0 ||
            it.roll === HitLocations.HitLocation.DEFAULT
          )
            it.roll = entry.roll;
        }
      });
    ...
    if (
      !!data.lastImport &&
      !!data.additionalresources.bodyplan &&
      bodyplan != data.additionalresources.bodyplan
    ) {
      let option = game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_IMPORT_BODYPLAN,
      );
      if (option == 1) {
        saveprot = false;
      }
  ...
  importPointTotalsFromGCSv2(total, atts, ads, skills, spells) {
    ...
    for (let i of ads)
      [p_ads, p_disads, p_quirks, p_race] = this.adPointCount(
        i,
        p_ads,
        p_disads,
        p_quirks,
    ...
    p_unspent -=
      p_atts + p_ads + p_disads + p_quirks + p_skills + p_spells + p_race;
    return {
      ...
      if (i.features?.length)
        for (let f of i.features) {
          if (f.type == 'reaction_bonus') {
            temp_r.push({
              modifier:
                f.amount * (f.per_level && !!i.levels ? parseInt(i.levels) : 1),
  ...
  recursiveGet(i) {
    ...
    let ch = [];
    if (i.children?.length)
      for (let j of i.children) ch = ch.concat(this.recursiveGet(j));
    if (i.modifiers?.length)
      for (let j of i.modifiers) ch = ch.concat(this.recursiveGet(j));
    if (!!i.disabled || (i.equipped != null && i.equipped == false)) return [];
    return [i].concat(ch);
  }

  adPointCount(i, ads, disads, quirks, race, toplevel = false) {
    if (i.type == 'advantage_container' && i.container_type == 'race')
      race += i.calc?.points;
    else if (
      i.type == 'advantage_container' &&
      i.container_type == 'alternative_abilities'
    )
      ads += i.calc?.points;
    else if (i.type == 'advantage_container' && !!i.children?.length) {
      var [a, d] = [0, 0];
      for (let j of i.children)
        [a, d, quirks, race] = this.adPointCount(j, a, d, quirks, race);
      if (toplevel) {
        if (a > 0) ads += a;
        else disads += a;
        disads += d;
  ...
  skPointCount(i, skills) {
    if (
      i.type == ('skill_container' || 'spell_container') &&
      i.children?.length
    )
  ...
  async importFromGCSv2(
    ...
    if (msg.length > 0) {
      ...
      let content = await renderTemplate(
        'systems/gurps/templates/chat-import-actor-errors.html',
        {
          lines: msg,
          version: version,
    ...
    try {
      ...
      commit = {
        ...commit,
        ...(await this.importAttributesFromGCSv2(
          r.attributes,
          r.equipment,
          r.calc,
        )),
      ...
      commit = {
        ...commit,
        ...(await this.importTraitsFromGCSv2(
          r.profile,
          r.created_date,
          r.modified_date,
        )),
    ...
    let deletes = Object.fromEntries(
      Object.entries(commit).filter(([key, value]) => key.includes('.-=')),
    );
    let adds = Object.fromEntries(
      Object.entries(commit).filter(([key, value]) => !key.includes('.-=')),
    );
    ...
    try {
      ...
      if (
        !game.settings.get(
          settings.SYSTEM_NAME,
          settings.SETTING_IGNORE_IMPORT_NAME,
        )
      ) {
      ...
      if (err.message == 'Maximum depth exceeded')
        msg.push(i18n('GURPS.importTooManyContainers'));
  ...
  async importFromGCSv1(xml, importname, importpath, suppressMessage = false) {
    ...
    let exit = false;
    if (!r) {
      if (importname.endsWith('.gca5'))
        msg.push(i18n('GURPS.importCannotImportGCADirectly'));
      if (importname.endsWith('.gca4'))
        msg.push(i18n('GURPS.importCannotImportGCADirectly'));
      else if (!xml.startsWith('<?xml'))
        msg.push(i18n('GURPS.importNoXMLDetected'));
      exit = true;
    } else {
      ...
      if (!!parsererror) {
        msg.push(
          i18n_f('GURPS.importErrorParsingXML', {
      ...
      isFoundryGCS =
        !!ra &&
        ra.release == 'Foundry' &&
        (ra.version == '1' || ra.version.startsWith('GCS'));
      isFoundryGCA =
        !!ra && ra.release == 'Foundry' && ra.version.startsWith('GCA');
      isFoundryGCA5 =
        !!ra && ra.release == 'Foundry' && ra.version.startsWith('GCA5');
    ...
    try {
      ...
      commit = {
        ...commit,
        ...this.importSkillsFromGCSv1(c.abilities?.skilllist, isFoundryGCS),
      };
      commit = { ...commit, ...this.importTraitsfromGCSv1(c.traits) };
      commit = {
        ...commit,
        ...this.importCombatMeleeFromGCSv1(
          c.combat?.meleecombatlist,
          isFoundryGCS,
        ),
      };
      commit = {
        ...commit,
        ...this.importCombatRangedFromGCSv1(
          c.combat?.rangedcombatlist,
          isFoundryGCS,
        ),
      };
      commit = {
        ...commit,
        ...this.importSpellsFromGCSv1(c.abilities?.spelllist, isFoundryGCS),
      };
      if (isFoundryGCS) {
        ...
        commit = {
          ...commit,
          ...this.importConditionalModifiersFromGCSv2(c.conditionalmods),
        };
      }
      if (isFoundryGCA) {
        commit = {
          ...commit,
          ...this.importLangFromGCA(c.traits?.languagelist),
        };
        commit = {
          ...commit,
          ...this.importAdsFromGCA(c.traits?.adslist, c.traits?.disadslist),
        };
        commit = {
          ...commit,
          ...this.importReactionsFromGCA(c.traits?.reactionmodifiers, vernum),
        };
      }
      ...
      commit = {
        ...commit,
        ...this.importNotesFromGCSv1(c.description, c.notelist),
      };
      commit = {
        ...commit,
        ...this.importEquipmentFromGCSv1(c.inventorylist, isFoundryGCS),
      };
      commit = {
        ...commit,
        ...(await this.importProtectionFromGCSv1(
          c.combat?.protectionlist,
          isFoundryGCA,
        )),
      };
    } catch (err) {
      console.log(err.stack);
      let msg = i18n_f('GURPS.importGenericError', {
        name: nm,
        error: err.name,
        message: err.message,
      });
  ...
  floatFrom(o) {
    ...
    if (!f) return 0;
    return f.includes(',')
      ? parseDecimalNumber(f, { thousands: '.', decimal: ',' })
      : parseDecimalNumber(f);
  }
  ...
  _findElementIn(list, uuid, name = '', mode = '') {
    ...
    recurselist(l, (e, k, d) => {
      if (
        (uuid && e.uuid == uuid) ||
        (!!e.name && e.name.startsWith(name) && e.mode == mode)
      )
        foundkey = k;
    });
    return foundkey == null
      ? foundkey
      : getProperty(this, list + '.' + foundkey);
  }
  ...
  importNotesFromGCSv1(descjson, json) {
    ...
    for (let key in json) {
      if (key.startsWith('id-')) {
        ...
        let j = json[key];
        let n =
          /** @type {Note & { imported: boolean, uuid: string, parentuuid: string }} */ (
            new Note()
          );
        //n.setNotes(t(j.text));
        n.notes = t(j.name);
  ...
  _getBodyPlan(locations) {
    ...
    locations.forEach(function (hitLocation) {
      tableNames.forEach(function (tableName) {
        if (
          HitLocations.hitlocationDictionary[tableName].hasOwnProperty(
            hitLocation.where,
          )
        ) {
    ...
    // in the table.
    let results = Object.keys(tableScores).filter(
      (it) => tableScores[it] === match,
    );
    if (results.length > 1) {
  ...
  importEquipmentFromGCSv1(json, isFoundryGCS) {
    ...
    for (let key in json) {
      if (key.startsWith('id-')) {
        ...
        let old = this._findElementIn('armor-builder.carried', eqt.uuid);
        if (!old) old = this._findElementIn('armor-builder.other', eqt.uuid);
  ...
  _removeOtf(key, text) {
    ...
    if (cnt == 0) {
      ...
      if ((front == '' || front.endsWith(' ')) && end.startsWith(' '))
        end = end.substring(1);
  ...
  async setResourceTrackers() {
    ...
    let templates = ResourceTrackerManager.getAllTemplates().filter(
      (it) => !!it.slot,
    );
  ...
  async applyTrackerTemplate(path, template) {
    ...
    template.tracker.value = template.tracker.isDamageTracker
      ? template.tracker.min
      : value;
  ...
  handleDamageDrop(damageData) {
    if (
      ...
      !game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_ONLY_GMS_OPEN_ADD,
      )
    )
  ...
  async handleItemDrop(dragData) {
    ...
    if (!data.globalid)
      await data.update({ _id: data._id, 'system.globalid': uuid });
    this.ignoreRender = true;
  ...
  // Drag and drop from an armor-builder list
  /**
  ...
  async handleEquipmentDrop(dragData) {
    ...
    if (
      ...
      ui.notifications?.warn(
        'You cannot transfer an Item that contains other armor-builder.',
      );
      return;
    }
    ...
    if (!!this.isOwner && !!srcActor.isOwner) {
      ...
      if (eqt.count < 2) {
        ...
        let content = await renderTemplate(
          'systems/gurps/templates/transfer-armor-builder.html',
          { eqt: eqt },
        );
        let callback = async (
          /** @type {JQuery<HTMLElement> | HTMLElement} */ html,
        ) => {
          // @ts-ignore
          ...
          if (!!destKey) {
            ...
            let item = /** @type {GurpsItem} */ (
              srcActor.items.get(eqt.itemid)
            );
      ...
      if (eqt.count > 1) {
        ...
        let callback = async (
          /** @type {HTMLElement | JQuery<HTMLElement>} */ html,
        ) =>
          // @ts-ignore
      ...
      let destowner = game.users?.players.find((p) =>
        this.testUserPermission(p, 'OWNER'),
      );
      if (!!destowner) {
  ...
  async updateItem(item) {
    ...
    let other = item.id
      ? await this._removeItemElement(item.id, 'armor-builder.other')
      : null; // try to remove from other
    if (!other) {
      // if not in other, remove from carried, and then re-add everything
      if (item.id) await this._removeItemElement(item.id, 'armor-builder.carried');
      await this.addItemData(item);
    } else {
      // If was in other... just add back to other (and forget addons)
      await this._addNewItemEquipment(
        item,
        'system.armor-builder.other.' + zeroFill(0),
      );
    }
  ...
  async addNewItemData(itemData, targetkey = null) {
    ...
    await this.updateEmbeddedDocuments('Item', [
      { _id: localItem.id, 'system.eqt.uuid': generateUniqueId() },
    ]);
    await this.addItemData(localItem, targetkey); // only created 1 item
  }

  // Once the Items has been added to our items list, add the armor-builder and any features
  /**
   * @param {ItemData} itemData
   * @param {string | null} [targetkey]
   */
  async addItemData(itemData, targetkey) {
    let [eqtkey, addFeatures] = await this._addNewItemEquipment(
      itemData,
      targetkey,
    );
    if (addFeatures) {
  ...
  // Make the initial armor-builder object (unless it already exists, saved in a user armor-builder)
  /**
   * @param {ItemData} itemData
   * @param {string | null} targetkey
   */
  async _addNewItemEquipment(itemData, targetkey) {
    ...
    if (!!_data.eqt.parentuuid) {
      ...
      recurselist(this.system.equipment.carried, (e, k, d) => {
        if (e.uuid == _data.eqt.parentuuid)
          found = 'system.armor-builder.carried.' + k;
      });
      if (!found)
        recurselist(this.system.equipment.other, (e, k, d) => {
          if (e.uuid == _data.eqt.parentuuid)
            found = 'system.armor-builder.other.' + k;
        });
      if (!!found) {
    ...
    if (targetkey == null)
      ...
      } else targetkey = 'system.armor-builder.other';
    if (targetkey.match(/^system\.equipment\.\w+$/))
      targetkey += '.' + zeroFill(0); //if just 'carried' or 'other'
    let eqt = _data.eqt;
    if (!eqt) {
      ui.notifications?.warn(
        'Item: ' +
  ...
  async _addItemAdditions(itemData, eqtkey) {
    ...
    commit = {
      ...commit,
      ...(await this._addItemElement(itemData, eqtkey, 'melee')),
    };
    commit = {
      ...commit,
      ...(await this._addItemElement(itemData, eqtkey, 'ranged')),
    };
    commit = {
      ...commit,
      ...(await this._addItemElement(itemData, eqtkey, 'ads')),
    };
    commit = {
      ...commit,
      ...(await this._addItemElement(itemData, eqtkey, 'skills')),
    };
    commit = {
      ...commit,
      ...(await this._addItemElement(itemData, eqtkey, 'spells')),
    };
  ...
  // called when armor-builder is being moved
  /**
   * @param {Equipment} eqt
   * @param {string} targetPath
   */
  async updateItemAdditionsBasedOn(eqt, targetPath) {
    await this._updateEqtStatus(
      eqt,
      targetPath,
      targetPath.includes('.carried'),
    );
  }
  ...
  async _updateEqtStatus(eqt, eqtkey, carried) {
    ...
    if (!!eqt.itemid) {
      ...
      await this.updateEmbeddedDocuments('Item', [
        {
          _id: item.id,
          'system.equipped': eqt.equipped,
          'system.carried': carried,
        },
      ]);
      if (!carried || !eqt.equipped)
  ...
  async moveEquipment(srckey, targetkey, shiftkey) {
    ...
    if (targetkey.match(/^system\.equipment\.\w+$/)) {
      ...
      if (!!object.itemid) {
        ...
        await this.updateEmbeddedDocuments('Item', [
          { _id: item.id, 'system.eqt.parentuuid': '' },
        ]);
      }
  ...
  async toggleExpand(path, expandOnly = false) {
    ...
    if (!!obj.collapsed && Object.keys(obj.collapsed).length > 0) {
      ...
      !expandOnly &&
      !!obj.contains &&
      Object.keys(obj.contains).length > 0
    ) {
      let temp = { ...obj.contains, ...obj.collapsed };
      let update = {
        [path + '.-=contains']: null,
  ...
  async _checkForMerging(srckey, targetkey) {
    ...
    if (
      ...
      this.ignoreRender = true;
      await this.updateEqtCount(
        targetkey,
        parseInt(srceqt.count) + parseInt(desteqt.count),
      );
  ...
  get _hitLocationRolls() {
    return HitLocations.HitLocation.getHitLocationRolls(
      this.system.additionalresources?.bodyplan,
    );
  }
  ...
  getTorsoDr() {
    ...
    let hl = Object.values(this.system.hitlocations).find(
      (h) => h.penalty == 0,
    );
    return !!hl ? hl : { dr: 0 };
  }
      ...
      Object.values(this.system.melee).forEach((melee) => {
        recurselist(this.system.equipment.carried, (e, k, d) => {
          if (
            ...
            !val &&
            e.equipped &&
            !!melee.name.match(makeRegexPatternFrom(e.name, false))
          ) {
  ...
  findEquipmentByName(pattern, otherFirst = false) {
     ...
     */
    var eqt, key;
    let list1 = otherFirst
      ? this.system.equipment.other
      : this.system.equipment.carried;
    let list2 = otherFirst
      ? this.system.equipment.carried
      : this.system.equipment.other;
    let pkey1 = otherFirst
      ? 'system.armor-builder.other.'
      : 'system.armor-builder.carried.';
    let pkey2 = otherFirst
      ? 'system.armor-builder.carried.'
      : 'system.armor-builder.other.';
    recurselist(
      list1,
  ...
  // Set the armor-builder count to 'count' and then recalc sums
  /**
   * @param {string} eqtkey
   * @param {number} count
   */
  async updateEqtCount(eqtkey, count) {
    ...
    if (
      game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_AUTOMATICALLY_SET_IGNOREQTY,
      )
    )
    ...
    if (!!eqt.itemid) {
      ...
      if (!!item)
        await this.updateEmbeddedDocuments('Item', [
          { _id: item.id, 'system.eqt.count': count },
        ]);
      else {
        ui.notifications?.warn(
    ...
    if (!!parent) {
      ...
      if (updatePuuid) {
        ...
        if (!!eqt.itemid) {
          ...
          if (item)
            await this.updateEmbeddedDocuments('Item', [
              { _id: item.id, 'system.eqt.parentuuid': puuid },
            ]);
        }
      }
    }
