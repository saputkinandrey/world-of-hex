...
} from '../hitlocation/hitlocation.js';
...
export class GurpsActorSheet extends ActorSheet {
  ...
  getData() {
    ...
    sheetData.conditionalEffectsTable =
      GURPS.ConditionalInjury.conditionalEffectsTable();
    GURPS.SetLastActor(this.actor);
    sheetData.eqtsummary = this.actor.system.eqtsummary;
    sheetData.navigateBar = {
      visible: game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_SHOW_SHEET_NAVIGATION,
      ),
    ...
    sheetData.effects =
      this.actor.getEmbeddedCollection('ActiveEffect').contents;
    sheetData.useQN = game.settings.get(
      settings.SYSTEM_NAME,
      settings.SETTING_USE_QUINTESSENCE,
    );
  ...
  activateListeners(html) {
    ...
    html
      .find('.dr button[data-key]')
      .on('click', this._onClickSplit.bind(this));
    ...
    html.find('button[data-operation="resource-reset"]').click((ev) => {
      ...
      let tracker = getProperty(this.actor.system, path);
      let value = !!tracker.isDamageTracker
        ? tracker.min || 0
        : tracker.max || 0;
    ...
    html
      .find('.tracked-resource .header.with-editor')
      .click(this.editTracker.bind(this));
    ...
    const updateActorWithChangedSeverity = (changedSeverity) => {
      ...
      this.actor.internalUpdate({
        'system.conditionalinjury.injury.severity':
          formatCIEmpty(changedSeverity),
        'system.conditionalinjury.injury.daystoheal': formatCIEmpty(
          CI.daysToHealForSeverity(changedSeverity),
        ),
      });
    };
    ...
    html.find('button[data-operation="ci-severity-inc"]').click(async (ev) => {
      ev.preventDefault();
      updateActorWithChangedSeverity(
        CI.incrementSeverity(
          this.actor.system.conditionalinjury.injury.severity,
        ),
      );
    });
    ...
    const updateActorWithChangedDaysToHeal = (changedDaysToHeal) => {
      ...
      this.actor.internalUpdate({
        'system.conditionalinjury.injury.severity': formatCIEmpty(
          CI.severityForDaysToHeal(changedDaysToHeal),
        ),
        'system.conditionalinjury.injury.daystoheal':
          formatCIEmpty(changedDaysToHeal),
      });
    };
    ...
    html.find('button[data-operation="ci-days-inc"]').click(async (ev) => {
      ...
      updateActorWithChangedDaysToHeal(
        CI.incrementDaysToHeal(
          this.actor.system.conditionalinjury.injury.daystoheal,
          ev.shiftKey ? 5 : 1,
        ),
      );
    });
    ...
    if (
      game.settings.get(settings.SYSTEM_NAME, settings.SETTING_ENHANCED_INPUT)
    ) {
    ...
    // On clicking armor-builder quantity increment, increase the amount.
    html.find('button[data-operation="armor-builder-inc"]').click(async (ev) => {
      ev.preventDefault();
      let parent = $(ev.currentTarget).closest('[data-key]');
      let path = parent.attr('data-key');
    ...
    html
      .find('button[data-operation="armor-builder-inc-uses"]')
      .click(async (ev) => {
        ev.preventDefault();
        let parent = $(ev.currentTarget).closest('[data-key]');
        let path = parent.attr('data-key');
    ...
    html
      .find('button[data-operation="armor-builder-dec-uses"]')
      .click(async (ev) => {
        ev.preventDefault();
        let parent = $(ev.currentTarget).closest('[data-key]');
        let path = parent.attr('data-key');
    ...
    // On clicking armor-builder quantity decrement, decrease the amount or remove from list.
    html.find('button[data-operation="armor-builder-dec"]').click(async (ev) => {
      ev.preventDefault();
      let parent = $(ev.currentTarget).closest('[data-key]');
      let path = parent.attr('data-key');
      let actor = this.actor;
          ...
          save: {
            ...
            callback: (html) => {
              ...
              actor.internalUpdate({
                'system.additionalresources.qnotes': i.value.replace(
                  /\n/g,
                  '<br>',
                ),
              });
    ...
    html
      .find('#open-modifier-popup')
      .on('click', this._showActiveEffectsListPopup.bind(this));
    html
      .find('#edit-move-modes')
      .on('click', this._showMoveModeEditorPopup.bind(this));

    html
      .find('#addFirstResourceTracker')
      .on('click', (ev) => this._addTracker());
  }
  ...
  _createEquipmentItemMenus(html) {
    ...
    let movedown = this._createMenu(
      ...
      this._moveEquipment.bind(this, 'system.armor-builder.other'),
    );
  ...
  _deleteItem(target) {
    ...
    if (key.includes('.armor-builder.')) this.actor.deleteEquipment(key);
  ...
  _isSortable(includeCollapsed, target) {
    ...
    if (includeCollapsed)
      return x?.collapsed && Object.keys(x.collapsed).length > 1;
    return false;
  }
  ...
  getMenuItems(elementid) {
    const map = {
      '#ranged': [
        this.sortAscendingMenu('system.ranged'),
        this.sortDescendingMenu('system.ranged'),
      ],
      '#melee': [
        this.sortAscendingMenu('system.melee'),
        this.sortDescendingMenu('system.melee'),
      ],
      '#advantages': [
        this.sortAscendingMenu('system.ads'),
        this.sortDescendingMenu('system.ads'),
      ],
      '#skills': [
        this.sortAscendingMenu('system.skills'),
        this.sortDescendingMenu('system.skills'),
      ],
      '#spells': [
        this.sortAscendingMenu('system.spells'),
        this.sortDescendingMenu('system.spells'),
      ],
      '#equipmentcarried': [
        this.addItemMenu(
          i18n('GURPS.armor-builder'),
          new Equipment(`${i18n('GURPS.armor-builder')}...`, true),
      ...
      '#equipmentother': [
        ...
        this.sortAscendingMenu('system.armor-builder.other'),
        this.sortDescendingMenu('system.armor-builder.other'),
      ],
    };
  ...
  makelistdrag(html, cls, type) {
    html.find(cls).each((i, li) => {
      ...
      li.addEventListener('dragstart', (ev) => {
        ...
        if (!!eqt.eqtkey) {
          eqtkey = eqt.eqtkey;
          eqt = GURPS.decode(this.actor, eqtkey); // Features added by armor-builder will point to the armor-builder
          type = 'equipment';
        }
  ...
  async _addNote(event) {
    ...
    let dlgHtml = await renderTemplate(
    ...
    let d = new Dialog(
      {
        ...
        buttons: {
          one: {
            label: 'Create',
            callback: async (html) => {
              ['notes', 'pageref', 'title'].forEach(
                (a) => (obj[a] = html.find(`.${a}`).val()),
              );
              let u = html.find('.save'); // Should only find in Note (or armor-builder)
  ...
  async editTracker(ev) {
    ...
    let path = $(ev.currentTarget)
      .closest('[data-gurps-resource]')
      .attr('data-gurps-resource');
    ...
    let selectTracker = async function (html) {
      ...
      let template = templates.find(
        (template) => template.tracker.name === name,
      );
    ...
    let d = new Dialog(
      {
        ...
        content: await renderTemplate(
          'systems/gurps/templates/actor/update-tracker.html',
          { templates: templates },
        ),
        buttons: buttons,
        default: 'edit',
  ...
  async editEquipment(actor, path, obj) {
    ...
    let dlgHtml = await renderTemplate(
      'systems/gurps/templates/armor-builder-editor-popup.html',
      obj,
    );
          ...
          one: {
            ...
            callback: async (html) => {
              [
                ...
                'notes',
                'pageref',
              ].forEach((a) => (obj[a] = html.find(`.${a}`).val()));
              ['count', 'cost', 'weight'].forEach(
                (a) => (obj[a] = parseFloat(html.find(`.${a}`).val())),
              );
              let u = html.find('.save'); // Should only find in Note (or armor-builder)
              if (!!u && obj.save != null) obj.save = u.is(':checked'); // only set 'saved' if it was already defined
              let v = html.find('.ignoreImportQty'); // Should only find in armor-builder
  ...
  async _onDrop(event) {
    ...
    if (dragData.type === 'equipment') {
      ...
      let dropTargetElements = $(event.target).closest(
        '.eqtdraggable, .eqtdragtarget',
      );
  ...
  get title() {
    ...
    return sheet === 'gurps.GurpsActorEditorSheet'
      ? '**** Editing: ' + t + ' ****'
      : t;
  }
  ...
  getCustomHeaderButtons() {
    ...
    const altsheet = game.settings.get(
      settings.SYSTEM_NAME,
      settings.SETTING_ALT_SHEET,
    );
    ...
    if (
      !game.settings.get(settings.SYSTEM_NAME, settings.SETTING_BLOCK_IMPORT) ||
      game.user.isTrusted
    )
      b.push({
        label: 'Import',
  ...
  async _onToggleSheet(event, altsheet) {
    event.preventDefault();
    let newSheet = Object.values(CONFIG.Actor.sheetClasses['character']).filter(
      (s) => s.label == altsheet,
    )[0].id;

    const original =
      this.actor.getFlag('core', 'sheetClass') ||
      Object.values(CONFIG.Actor.sheetClasses['character']).filter(
        (s) => s.default,
      )[0].id;
  ...
  async _onRightClickOtf(event) {
    ...
    if (isDamageRoll) {
      ...
      GURPS.whisperOtfToOwner(
        event.currentTarget.dataset.otf,
        null,
        event,
        !isDamageRoll,
        this.actor,
      ); // Can't blind roll damages (yet)
    }
  }
...
export class GurpsActorEditorSheet extends GurpsActorSheet {
  ...
  makeDeleteMenu(html, cssclass, obj, eventname = 'contextmenu') {
    new ContextMenu(html, cssclass, this.deleteItemMenu(obj), {
      eventName: eventname,
    });
  }

  makeHeaderMenu(html, cssclass, name, obj, path, eventname = 'contextmenu') {
    new ContextMenu(html, cssclass, [this.addItemMenu(name, obj, path)], {
      eventName: eventname,
    });
  }
  ...
  activateListeners(html) {
    ...
    html
      .find('#ignoreinputbodyplan')
      .on('click', this._onClickIgnoreImportBodyPlan.bind(this));
    html
      .find('label[for="ignoreinputbodyplan"]')
      .on('click', this._onClickIgnoreImportBodyPlan.bind(this));

    html
      .find('#showflightmove')
      .on('click', this._onClickShowFlightMove.bind(this));
    html
      .find('label[for="showflightmove"]')
      .on('click', this._onClickShowFlightMove.bind(this));
      ...
      if (bodyplan !== this.actor.system.additionalresources.bodyplan) {
        ...
        if (!hitlocationTable) {
          ...
          for (let loc in hitlocationTable) {
            ...
            let originalLoc = Object.values(oldlocations).filter(
              (it) => it.where === loc,
            );
  ...
  async _onClickIgnoreImportBodyPlan(ev) {
    ...
    let ignore = !current;
    await this.actor.update({
      'system.additionalresources.ignoreinputbodyplan': ignore,
    });
  }
  ...
  async _onClickShowFlightMove(ev) {
    ...
    let show = !current;
    await this.actor.update({
      'system.additionalresources.showflightmove': show,
    });
  }
}
