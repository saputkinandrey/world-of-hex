...
export class EffectModifierControl {
  ...
  constructor() {
    ...
    Hooks.on(
      ...
      this._createEffectModifierButton.bind(this),
    );
  ...
  togglePopup(closeOptions) {
    ...
    let toggle = $.find(
      '[data-control=token] ol > li[data-tool=GURPSEffectsMod]',
    );
  ...
  _registerSetting() {
    game.settings.register(
      SYSTEM_NAME,
      EffectModifierControl.SETTING_SHOW_EFFECTMODIFIERS,
      {
        name: i18n(
          'GURPS.settingTokenToolsShowEffectMods',
          'Show Effect Modifiers',
        ),
        hint: i18n(
        ...
        onChange: (value) =>
          console.log(
            `${EffectModifierControl.SETTING_SHOW_EFFECTMODIFIERS} : ${value}`,
          ),
      },
    );
  }
  ...
  _createActiveEffect(effect, _, __) {
    ...
    console.debug(
      `_createActiveEffect: effect id: ${effectID}, token actor id: ${sharedStateID}`,
    );
  ...
  _updateToken(tokenDocument) {
    ...
    console.debug(
      `_updateToken: token id: ${tokenID}, token actor id: ${sharedStateID}`,
    );
