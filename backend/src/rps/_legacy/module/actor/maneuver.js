...
const oldTemporaryEffects = Object.getOwnPropertyDescriptor(
  Actor.prototype,
  'temporaryEffects',
);
  ...
  get: function () {
    ...
    if (!!results && results.length > 1) {
      ...
      const maneuvers = results.filter(
        (/** @type {ActiveEffect} */ e) =>
          e.getFlag('core', 'statusId') === MANEUVER,
      );
...
export default class Maneuvers {
  ...
  static isActiveEffectManeuver(activeEffect) {
    return activeEffect.getFlag
      ? activeEffect.getFlag('core', 'statusId') === MANEUVER
      : false;
  }
  ...
  static getActiveEffectManeuvers(effects) {
    return effects
      ? effects.filter((it) => Maneuvers.isActiveEffectManeuver(it))
      : [];
  }
}
