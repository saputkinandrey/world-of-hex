...
export default class MoveModeEditor extends Application {
  ...
  get title() {
    ...
    return i18n_f(
      'GURPS.moveModeEditor.Title',
      { name: name },
      'Movement Modes for {name}',
    );
  }
  ...
  activateListeners(html) {
    ...
    html
      .find('.move-mode-control')
      .on('change click keyup', this._onEffectControl.bind(this, html));
  }
      ...
      case 'default':
        {
          ...
          if (!state) {
            ...
            for (const k in this.moveData)
              json.push(`"system.move.${k}.default": ${key === k}`);
            let text = '{ ' + json.join(',') + ' }';
