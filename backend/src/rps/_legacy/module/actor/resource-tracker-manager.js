...
export class ResourceTrackerManager extends FormApplication {
  static initSettings() {
    game.settings.registerMenu(
      ...
      settings.SETTING_TRACKER_DEFAULT_EDITOR,
      {
    ...
    game.settings.register(
      settings.SYSTEM_NAME,
      settings.SETTING_TRACKER_TEMPLATES,
      {
        ...
        onChange: (value) =>
          console.log(
            `Updated Default Resource Trackers: ${JSON.stringify(value)}`,
          ),
      },
    );
  }
  ...
  _updateObject() {
    ...
    let entries = Object.entries(GURPS.DamageTables.woundModifiers).filter(
      ([k, v]) => !!v.resource,
    );
    entries.forEach(
      ([key, _]) => delete GURPS.DamageTables.woundModifiers[key],
    );
    entries.forEach(([key, _]) => {
      let toDelete = Object.entries(GURPS.DamageTables.damageTypeMap).filter(
        ([k, v]) => v === key,
      );
