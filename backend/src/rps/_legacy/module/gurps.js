...
import {
  parselink,
  parseForRollOrDamage,
  COSTS_REGEX,
} from '../../lib/parselink.js';
...
import { ItemImporter } from './/item-import.js';
...
`;
  ...
  GURPS.setLastTargetedRoll = function (
    chatdata,
    actorid,
    tokenid,
    ...
    return new Promise((resolve, reject) => {
      ...
      if (
        game.settings.get(
          ...
          Settings.SETTING_IMPORT_FILE_ENCODING,
        ) == 1
      )
  ...
  async function executeOTF(string, priv = false, event = null, actor = null) {
    ...
    if (!!action.action) {
      ...
      let result = await GURPS.performAction(
        action.action,
        actor || GURPS.LastActor,
        event,
      );
      answer = !!result;
    } else
      ui.notifications.warn(
        `"${string}" did not parse into a valid On-the-Fly formula`,
      );
    return answer;
  }
    ...
    if (!!action.floatingAttribute) {
      if (!!actor) {
        ...
        skillLevel =
          valueText === rsl
            ? parseInt(value)
            : parseInt(valueText) + parseInt(value);
      } else {
        ui.notifications?.warn(
          'You must have a character selected to use a "Based" Skill',
        );
      }
    }
  ...
  const actionFuncs = {
    ...
    async chat({ action, actor, event }) {
      ...
      let ret = await GURPS.ChatProcessors.startProcessingLines(
        chat,
        event?.chatmsgData,
        event,
      );
    ...
    damage({ action, event, actor, targets }) {
      ...
      if (!!action.mod)
        GURPS.ModifierBucket.addModifier(action.mod, action.desc); // special case where Damage comes from [D:attack + mod]
    ...
    deriveddamage({ action, event, actor, targets }) {
      ...
      let df = action.derivedformula.match(/sw/i)
        ? actor.system.swing
        : actor.system.thrust;
      // action fails if there's no formula
      if (!df) {
        ui.notifications?.warn(
          `${actor.name} does not have a ${action.derivedformula.toUpperCase()} formula`,
        );
        return false;
      }
    ...
    attackdamage({ action, event, actor, targets }) {
      ...
      let att = null;
      att = GURPS.findAttack(
        actor.system,
        action.name,
        !!action.isMelee,
        !!action.isRanged,
      ); // find attack possibly using wildcards
      if (!att) {
        ui.notifications.warn(
    ...
    derivedroll({ action, actor, event }) {
      ...
      let df = action.derivedformula.match(/[Ss][Ww]/)
        ? actor.system.swing
        : actor.system.thrust;
      if (!!action.costs) GURPS.ModifierBucket.addModifier(0, action.costs);
    ...
    async attack({ action, actor, event }) {
      ...
      if (
        opt.obj.checkotf &&
        !(await GURPS.executeOTF(opt.obj.checkotf, false, event, actor))
      )
        return false;
      if (opt.obj.duringotf)
        await GURPS.executeOTF(opt.obj.duringotf, false, event, actor);
      if (!!action.costs) GURPS.ModifierBucket.addModifier(0, action.costs);
      if (!!action.mod)
        GURPS.ModifierBucket.addModifier(action.mod, action.desc, targetmods);
      if (action.overridetxt)
        opt.text +=
          "<span style='font-size:85%'>" + action.overridetxt + '</span>';
    ...
    ['weapon-block']({ action, actor, event }) {
      ...
      if (isNaN(target) || target == 0) {
        ui.notifications.warn(
          `No Block for '${action.name.replace('<', '&lt;')}' found on ${actor.name}`,
        );
        return false;
      }
      ...
      let aid = actor ? `@${actor.id}@` : '';
      const chatthing =
        thing === '' ? att.name + mode : `[${aid}B:"${thing}${mode}"]`;
    ...
    ['weapon-parry']({ action, actor, event, calcOnly }) {
      ...
      if (isNaN(target) || target == 0) {
        ui.notifications.warn(
          `No Parry for '${action.name.replace('<', '&lt;')}' found on ${actor.name}`,
        );
        return false;
      }
      const thing = att.name
      ...
      let aid = actor ? `@${actor.id}@` : '';
      const chatthing =
        thing === '' ? att.name + mode : `[${aid}P:"${thing}${mode}"]`;
    ...
    async attribute({ action, actor, event, originalOtf, calcOnly }) {
      ...
      if (!!action.mod)
        ...
        opt.text = "<span style='font-size:85%'>" + action.desc + '</span>';
      if (action.overridetxt)
        opt.text +=
          "<span style='font-size:85%'>" + action.overridetxt + '</span>';
  ...
  async function findBestActionInChain({
    action,
    actor,
    event,
    targets,
  ...
  async function performAction(action, actor, event = null, targets = []) {
    ...
    if (['attribute', 'skill-spell'].includes(action.type)) {
      action = await findBestActionInChain({
        action,
        event,
        actor,
        targets,
    ...
    return !action
      ? false
      : await GURPS.actionFuncs[action.type]({
          action,
          actor,
          event,
          targets,
  ...
  function findSkillSpell(
    ...
    let skillRegExp = new RegExp(
      removeOtf + makeRegexPatternFrom(sname, false, false),
      'i',
    );
    let best = 0;
    if (!isSpellOnly)
  ...
  async function handleRoll(event, actor, options) {
    ...
    if ('damage' in element.dataset) {
      ...
      for (let part of parts) {
        ...
        if (result?.action) {
          if (options?.combined && result.action.type == 'damage')
            result.action.formula = multiplyDice(
      ...
      let text = /** @type {string} */ (
        element.dataset.name || element.dataset.otf
      );
  ...
  /**
   *  Funky helper function to be able to list hierarchical armor-builder in a linear list (with appropriate keys for editing)
   * @param {Record<string, any>} eqts
   * @param {{ fn: (arg0: any, arg1: { data: any; }) => string; }} options
   * @param {number} level
   * @param {{ indent: any; key: string; count: any; }} data
   * @param {string=} parentkey
   * @param {{ armor-builder: { carried: Object; }; }|null} src
   */
  function listeqtrecurse(
    eqts,
    options,
    level,
    data,
    ...
    for (let key in eqts) {
      ...
      if (
        ...
        game.settings.get(
          Settings.SYSTEM_NAME,
          Settings.SETTING_REMOVE_UNEQUIPPED,
        )
      ) {
  ...
  GURPS.whisperOtfToOwner = function (
    otf,
    overridetxt,
    event,
    blindcheck,
  ...
  GURPS.resolveDamageRoll = function (
    event,
    actor,
    otf,
    overridetxt,
    ...
    buttons.multiple = {
      ...
      callback: (html) => {
        ...
        if (isOtf)
          GurpsWiring.handleGurpslink(event, actor, null, { targets: targets });
    ...
    buttons.combined = {
      ...
      callback: (html) => {
        ...
        if (isOtf)
          GurpsWiring.handleGurpslink(event, actor, null, { combined: number });
  ...
  Hooks.once('init', async function () {
    ...
    let src =
      game.i18n.lang == 'pt_br'
        ? 'systems/gurps/icons/gurps4e-pt_br.webp'
        : 'systems/gurps/icons/gurps4e.webp';
    ...
    Hooks.on('renderSidebarTab', async (app, html) => {
      // Add the import armor-builder button...
            ...
            new Dialog(
              {
                ...
                content: await renderTemplate(
                  'systems/gurps/templates/item-import.html',
                ),
                buttons: {
                  import: {
      ...
      const chat = html[0]?.querySelector(
        hasMeme ? '.EasyMDEContainer' : '#chat-message',
      );
      ...
      if (!!chat)
        chat.addEventListener('drop', (event) => dropHandler(event, false));
    ...
    game.keybindings.register('gurps', 'toggleDiceDisplay', {
      ...
      onDown: () => {
        if (
          game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_CTRL_KEY)
        ) {
          GURPS.savedRollMode = game.settings.get('core', 'rollMode');
          game.settings.set(
            'core',
            'rollMode',
            game.user?.isGM ? 'gmroll' : 'blindroll',
          );
        }
      },
      onUp: () => {
  ...
  Hooks.once('ready', async function () {
    ...
    // Test for migration
    let mv = game.settings.get(
      Settings.SYSTEM_NAME,
      Settings.SETTING_MIGRATION_VERSION,
    );
    let quiet = false;
    if (!mv) {
      mv = '0.0.1';
    ...
    // @ts-ignore
    console.log(
      'Current Version: ' + GURPS.currentVersion + ', Migration version: ' + mv,
    );
...
<div style='font-size:70%'>
  ...
  <div style='text-align: center;'>
    ...
    <div><a href="https://ko-fi.com/crnormand"><img height="24" src="systems/gurps/icons/SupportMe_stroke@2x.webp"></a></div>
      ...
      if (
        game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_SHOW_CHANGELOG)
      ) {
    ...
    resourceTrackers.forEach(
      (it) => (GURPS.DamageTables.damageTypeMap[it.alias] = it.alias),
    );
    resourceTrackers.forEach(
      (it) =>
      ...
      if (!!oldmacro && !!oldmacro.getFlag('gurps', 'drag-drop-otf')) {
        ...
        new Dialog({
          ...
          content: `Merge both macros into this:<br><br><mark>${oldmacro.command.split('\n').join('<br>')}<br>${cmd
            ...
            .join(
              '<br>',
            )}</mark><br><br>Or just replace current macro with:<br><br><mark>${c
            .split('\n')
            .join('<br>')}</mark><br>&nbsp;<br>`,
          buttons: {
            one: {
            ...
            two: {
              ...
              label: 'Replace',
              callback: () =>
                setmacro(name, (!!data.bucket ? '/clearmb\n' : '') + cmd),
            },
          },
          default: 'one',
        }).render(true);
    ...
    Hooks.on('renderCombatTracker', function (a, html, c) {
      ...
      if (!html.hasClass('bound')) {
        ...
        html.on('drop', function (ev) {
          ...
          // @ts-ignore
          let combatant = $(elementMouseIsOver)
            .parents('.combatant')
            .attr('data-combatant-id');
          // @ts-ignore
          let target = game.combat.combatants.filter(
            (c) => c.id === combatant,
          )[0];
    ...
    game.socket.on('system.gurps', async (resp) => {
      if (resp.type == 'updatebucket') {
        if (resp.users.includes(game.user.id)) {
          if (resp.add) {
            resp.bucket.modifierList.forEach((e) =>
              GURPS.ModifierBucket.addModifier(e.mod, e.desc),
            );
      ...
      if (resp.type == 'executeOtF') {
        ...
        if (
          game.users.isGM ||
          (resp.users.length > 0 && !resp.users.includes(game.user.name))
        )
          return;
        // @ts-ignore
      ...
      if (resp.type == 'dragEquipment1') {
        ...
        Dialog.confirm({
          ...
          yes: () => {
            // @ts-ignore
            let destKey = destactor._findEqtkeyForId(
              'name',
              resp.itemData.name,
            );
            if (!!destKey) {
      ...
      if (resp.type == 'dragEquipment3') {
        ...
        ui.notifications.info(
          `${destActor.name} did not want ${resp.itemname}`,
        );
      }
    });
    ...
    if (game.i18n.lang != 'en') {
      console.log(
        'Mapping ' + game.i18n.lang + ' translations into PARSELINK_MAPPINGS',
      );
    ...
    Object.values(CONFIG.Actor.sheetClasses['character']).forEach((e) => {
      if (
        e.id.toString().startsWith(Settings.SYSTEM_NAME) &&
        e.id != 'gurps.GurpsActorSheet'
      )
    ...
    game.settings.register(Settings.SYSTEM_NAME, Settings.SETTING_ALT_SHEET, {
      ...
      onChange: (value) =>
        console.log(`${Settings.SETTING_ALT_SHEET}: ${value}`),
    });
