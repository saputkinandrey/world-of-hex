...
export class CompositeDamageCalculator {
  ...
  constructor(defender, damageData) {
    this._useBluntTrauma = game.settings.get(
      settings.SYSTEM_NAME,
      settings.SETTING_BLUNT_TRAUMA,
    );
    this._useLocationModifiers = game.settings.get(
      settings.SYSTEM_NAME,
      settings.SETTING_LOCATION_MODIFIERS,
    );
    this._useArmorDivisor = game.settings.get(
      settings.SYSTEM_NAME,
      settings.SETTING_APPLY_DIVISOR,
    );
    ...
    this._calculators = damageData.map(
      (data) => new DamageCalculator(this, data),
    );

    this.viewId = this._calculators.length == 1 ? 0 : 'all';

    this._defaultWoundingModifiers = Object.keys(
      GURPS.DamageTables.woundModifiers,
    ).reduce(function (r, e) {
      if (!GURPS.DamageTables.woundModifiers[e].nodisplay)
        r[e] = GURPS.DamageTables.woundModifiers[e];
      return r;
    }, {});
    ...
    if (
      Object.keys(this._defaultWoundingModifiers).includes(
        damageData[0].damageType,
      )
    )
    ...
    let damageLocation = !!damageData[0].hitlocation
      ? damageData[0].hitlocation.toLowerCase()
      : '';
    ...
    if (!found) {
      ...
      found = values.find((value) => {
        let found = !!(
          ...
          (value.name === 'Injury Tolerance' &&
            value.notes.includes('Unliving'))
        );
  ...
  isHomogenous(values, found) {
    if (!found) {
      ...
      found = values.find((value) => {
        let found = !!(
          ['Injury Tolerance (Homogenous)', 'Homogenous'].includes(
            value.name,
          ) ||
          (value.name === 'Injury Tolerance' &&
            value.notes.includes('Homogenous'))
        );
  ...
  static isResourceDamageType(damageType) {
    ...
    return (
      !!modifier && !!GURPS.DamageTables.woundModifiers[damageType].resource
    );
  }
  ...
  get showApplyAction() {
    return (
      game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_DEFAULT_ADD_ACTION,
      ) == 'apply' ||
      (game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_DEFAULT_ADD_ACTION,
  ...
  get basicDamage() {
    if (this._viewId === 'all')
      return this._calculators.reduce((sum, a) => sum + a._basicDamage, 0);
  ...
  get DR() {
    ...
    return this._hitLocation === 'Large-Area'
      ...
      : HitLocationEntry.findLocation(entries, this._hitLocation).getDR(
          this.damageType,
        );
  }
  ...
  get effects() {
    ...
    let effects = [];
    this._calculators
      .map((calculator) => calculator.effects)
  ...
  get effectiveArmorDivisor() {
    ...
    if (
      (this._armorDivisor > 1 || this._armorDivisor === -1) &&
      this._isHardenedDR
    ) {
  ...
  get effectiveWoundModifiers() {
    ...
    if (this._useLocationModifiers) {
      switch (this._hitLocation) {
        ...
        default: {
          if (
            [hitlocation.EXTREMITY, hitlocation.LIMB].includes(
              this.hitLocationRole,
            )
          )
  ...
  get hasAdditionalWoundingModifiers() {
    return (
      this.additionalWoundModifier > 0 ||
      this.effectiveVulnerabilityMultiple > 0
    );
  }
  ...
  get isWoundModifierAdjustedForInjuryTolerance() {
    ...
    let entries = Object.keys(table).filter(
      (key) => table[key].changed === 'injury-tolerance',
    );
  ...
  get isWoundModifierAdjustedForLocation() {
    let table = this.effectiveWoundModifiers;
    let entries = Object.keys(table).filter(
      (key) => table[key].changed === 'hitlocation',
    );
    return entries.length > 0;
  }
  ...
  get isWoundModifierAdjustedForDamageType() {
    let table = this.effectiveWoundModifiers;
    let entries = Object.keys(table).filter(
      (key) => table[key].changed === 'damagemodifier',
    );
    return entries.length > 0;
  }
  ...
  get locationMaxHP() {
    ...
    if (this.hitLocationRole === hitlocation.EXTREMITY)
      return this.HP.max / 3 + 1;
  ...
  get pointsToApply() {
    return this._calculators
      .map((it) => it.pointsToApply)
      .reduce((acc, value) => acc + value);
  }

  get resource() {
    // if (CompositeDamageCalculator.isResourceDamageType(this._applyTo)) {
    let trackers = objectToArray(
      this._defender.system.additionalresources.tracker,
    );
    let tracker = null;
    let index = null;
    ...
    if (!!tracker)
      return [
        tracker,
        `system.additionalresources.tracker.${zeroFill(index, 4)}`,
      ];
    // }
  ...
  get totalBasicDamage() {
    return this._calculators
      .map((it) => it.basicDamage)
      .reduce((acc, value) => acc + value);
  }

  get totalWoundingModifier() {
    return (
      (this.woundingModifier + this._additionalWoundModifier) *
  ...
  get woundingModifier() {
    ...
    if (this._damageType === 'User Entered')
      return this._userEnteredWoundModifier;
  ...
  async randomizeHitLocation() {
    ...
    let loc = this._defender.hitLocationsWithDR.filter((it) =>
      it.roll.includes(total),
    );
...
class DamageCalculator {
  ...
  get injury() {
    ...
    if (
      this._parent._damageReductionLevel !== null &&
      this._parent._damageReductionLevel != 0
    ) {
      // Injury Tolerance (Damage Reduction) can't reduce damage below 1
      injury = Math.max(
        1,
        Math.floor(injury / this._parent._damageReductionLevel),
      );
    }

    // B380: A target with Injury Tolerance (Diffuse) is even harder to damage!
    if (
      this._parent.isInjuryTolerance &&
      this._parent.injuryToleranceType === DIFFUSE
    ) {
  ...
  get calculatedBluntTrauma() {
    ...
    if (this._parent.damageType === 'cr')
      return Math.floor(this.effectiveDamage / 5);
    return Math.floor(this.effectiveDamage / 10);
  }

  get effectiveBluntTrauma() {
    return this._bluntTrauma === null
      ? this.calculatedBluntTrauma
      : this._bluntTrauma;
  }
  ...
  get penaltyToHTRollForStunning() {
    ...
    if (
      isInjuryTolerance &&
      (injuryToleranceType === DIFFUSE || injuryToleranceType === HOMOGENOUS)
    ) {
      return 0;
    }
    ...
    if (
      this.isMajorWound &&
      ['Face', 'Vitals', 'Groin'].includes(this._parent.hitLocation)
    )
      return 5;
    ...
    if (
      this.isMajorWound &&
      ['Eye', 'Skull', 'Brain'].includes(this._parent.hitLocation)
    )
      return 10;
  ...
  get isCripplingInjury() {
    if (
      ...
      this._parent.isCrippleableLocation
    ) {
  ...
  get isInjuryReducedByLocation() {
    return (
      this._parent.useLocationModifiers &&
      this.isCripplingInjury &&
      this._parent.hitLocation !== 'Eye'
    );
  }
  ...
  get isKnockbackEligible() {
    ...
    if (
      (this._parent.damageType === 'cr' || this._parent.damageType === 'kb') &&
      this._basicDamage > 0
    )
      return true;

    return (
      this._parent.damageType === 'cut' &&
      this._basicDamage > 0 &&
