...
export default class DamageChat {
        ...
        button.on('click', (ev) => {
          ...
          if (!!actor) actor.handleDamageDrop(transfer.payload);
            ...
            ui.notifications?.warn(
              'Unable to find token with ID:' + transfer.userTarget,
            );
        });
      }
    }
  ...
  static async _dropCanvasData(canvas, dropData) {
    if (
      ...
      let handle = (/** @type {GurpsActor} */ actor) =>
      ...
      let d = new Dialog(
        {
          ...
          content: await renderTemplate(
            'systems/gurps/templates/apply-damage/select-token.html',
            {
              tokens: targets,
            },
          ),
  ...
  static async create(
    ...
    if (!!event && event.data?.repeat > 1)
      for (let i = 0; i < event.data.repeat; i++) tokenNames.push('' + i);

    if (tokenNames.length == 0) tokenNames.push('');

    let draggableData = [];
    for (const tokenName of tokenNames) {
      let data = await message._createDraggableSection(
        actor,
        dice,
        tokenName,
        targetmods,
  ...
  _getDiceData(
    ...
    let divisor = !!result.groups?.divisor
      ? parseFloat(result.groups.divisor)
      : 0;
  ...
  async _createDraggableSection(
    ...
    let roll = /** @type {GurpsRoll} */ (
      Roll.create(diceData.formula + `+${diceData.modifier}`)
    );
    ...
    if (
      (roll.dice.length > 0 && roll.dice[0].results.length > 1) ||
      diceData.adds1 !== 0
    ) {
      let tempString = '';
    ...
    let contentData = {
      ...
      damageTypeText:
        diceData.damageType === 'dmg' ? ' ' : `'${diceData.damageType}' `,
  ...
  async _createChatMessage(actor, diceData, targetmods, draggableData, event) {
    ...
    damageType = !!diceData.extdamagetype
      ? `${damageType} ${diceData.extdamagetype}`
      : damageType;

    let html = await renderTemplate(
      'systems/gurps/templates/damage-message.hbs',
      {
        ...
        modifiers: targetmods.map(
          (it) => `${it.mod} ${it.desc.replace(/^dmg/, 'damage')}`,
        ),
