...
export default class ApplyDamageDialog extends Application {
  ...
  constructor(actor, damageData, options = {}) {
    ...
    this.actor = actor;
    this.isSimpleDialog = game.settings.get(
      settings.SYSTEM_NAME,
      settings.SETTING_SIMPLE_DAMAGE,
    );
    this.timesToApply = 1;

    let trackers = objectToArray(actor._additionalResources.tracker);
    this._resourceLabels = trackers
      .filter((it) => !!it.isDamageType)
      .filter((it) => !!it.alias);
  ...
  activateListeners(html) {
    ...
    html
      .find('.decimal-digits-only')
      .inputFilter((value) => digitsAndDecimalOnly.test(value));
    ...
    html
      .find('#basicDamage')
      .on('change', (ev) =>
        this._updateModelFromInputText(
          $(ev.currentTarget),
          'basicDamage',
          parseIntFrom,
        ),
    ...
    html.find('#apply-to').on('change', (ev) => {
      this._calculator.applyTo = $(ev.currentTarget)
    ...
    html
      ...
      .on('click', async () => this._randomizeHitLocation());
    ...
    html
      ...
      .click((ev) =>
        this._updateModelFromCheckedElement(
          $(ev.currentTarget),
          'useArmorDivisor',
        ),
      );
    ...
    html
      ...
      .click((ev) =>
        this._updateModelFromRadioValue(
          $(ev.currentTarget),
    ...
    html.find('#blunt-trauma-field input').on('change', (ev) => {
      ...
      this._calculator.bluntTrauma =
        currentValue === '' ||
        currentValue === this._calculator.calculatedBluntTrauma
          ? null
    ...
    html
      .find('#shotgun-damage')
      .click((ev) =>
        this._updateModelFromCheckedElement($(ev.currentTarget), 'isShotgun'),
      );
    ...
    // ==== Results ====
    html
      .find('#result-effects button')
      .click(async (ev) => this._handleEffectButtonClick(ev));
  ...
  async _renderTemplate(template, data) {
    return renderTemplate(
      'systems/gurps/templates/apply-damage/' + template,
      data,
    );
  }
  ...
  async _handleEffectButtonClick(ev) {
    ...
    if (object.type === 'knockback') {
      ...
      let acroCheck =
        object.modifier === 0 ? acro : `${acro}-${object.modifier}`;
      let judo = i18n('GURPS.skillJudo');
      let judoCheck =
        object.modifier === 0 ? judo : `${judo}-${object.modifier}`;
    ...
    if (
      game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_WHISPER_STATUS_EFFECTS,
      )
    ) {
  ...
  async resolveInjury(keepOpen, injury, publicly, results = null) {
    ...
    let data = {
      ...
      location:
        this._calculator.resourceType === 'HP'
    ...
    let newValue = resource.isDamageTracker
      ? resource.value + injury
      : resource.value - injury;
