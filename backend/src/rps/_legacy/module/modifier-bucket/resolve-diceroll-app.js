...
export default class ResolveDiceRoll extends Application {
   ...
   */
  getData(options) {
    const data = /** @type {ResolveDiceRollData}*/ (
      /** type {undefined} */ super.getData(options)
    );
  ...
  activateListeners(html) {
    ...
    html
      .find('input')
      .inputFilter((value) => commaSeparatedNumbers.test(value));
    ...
    html.find('input').on('change', (ev) => {
      let diceTerm = this.diceTerms.find(
        (it) => it.term.id === ev.currentTarget.id,
      );
    ...
    html
      ...
      .on(
        ...
        (ev) => {
          ...
          let valid = !!diceTerm
            ? this.isValid(diceTerm.term, target.value)
            : false;
          ...
          if (!!diceTerm) {
            if (valid) {
              ...
              target.setSelectionRange(
                target.oldSelectionStart || null,
                target.oldSelectionEnd || null,
              );
            } else {
              target.value = '';
            }
  ...
  isValid(term, text) {
    ...
    if (!this.isIndividualDieResults(text)) {
      ...
      return (
        value >= minPerDie * term.number && value <= maxPerDie * term.number
      );
    }
  ...
  getValues(diceTerm) {
    ...
    return this.generate(
      { number: diceTerm.term.number, faces: diceTerm.term.faces },
      target,
    );
  }
