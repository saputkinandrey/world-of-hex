...
export class SlamCalculator {
  ...
  */
  async process(data) {
    ...
    let attackerRoll = Roll.create(
      diceToFormula(attackerDice, `[Slam Attacker's roll]`, true),
    );
    ...
    let targetRoll = Roll.create(
      diceToFormula(targetDice, `[Slam Defender's roll]`, true),
    );
    ...
    let html = await renderTemplate(
      'systems/gurps/templates/slam-results.html',
      {
  ...
  targetDXCheck(attackerResult, targetResult) {
    return (
      attackerResult >= targetResult &&
      !this.targetFallsDown(attackerResult, targetResult)
    );
  }
  ...
  explainDieRoll(roll, isAoAStrong = false, shieldDB = 0) {
    ...
    let explanation =
      roll.terms.length > 1
        ? `${i18n('GURPS.rolled')} (${results})`
        : `${i18n('GURPS.rolled')} ${results}`;
    if (roll.terms[2] !== '0')
      explanation += `${roll.terms[1].formula}${roll.terms[2].formula}`;

    if (!!isAoAStrong) explanation += ` + 2 (${i18n('GURPS.slamAOAStrong')})`;
    if (!!shieldDB)
      explanation += ` + ${shieldDB} (${i18n('GURPS.slamShieldDB')})`;
    return explanation;
  }
}
