...
export default class StatusChatProcessor extends ChatProcessor {
  ...
  async process(_) {
    if (!this.match)
      throw new Error(
        'match does not exist! Did you call ChatProcessor.matches()?',
      );
    ...
    let tokenName =
      !self && !!this.match.groups?.target
        ? this.match.groups.target.replace(/^:(.*)$/, '$1')
        : null;
    ...
    if (!tokens || tokens.length === 0) {
      ui.notifications.warn(
        i18n_f('GURPS.chatSelectSelfOrNameTokens', { self: '@self' }),
      );
      return;
    }
    ...
    if (!effect) {
      if (!effectText) {
        ...
        !effectText.match(
          new RegExp(makeRegexPatternFrom(GURPS.StatusEffectStanding), 'i'),
        ) &&
        !effectText.match(
          new RegExp(
            makeRegexPatternFrom(i18n(GURPS.StatusEffectStandingLabel)),
            'i',
          ),
        )
      ) {
        ui.notifications.warn(
          i18n('GURPS.chatNoStatusMatched') + " '" + effectText + "'",
        );
        return;
      }
      isStanding = true;
    ...
    else if (theCommand == Command.unset)
      return await this.unset(tokens, effect);
    else
      ui.notifications.warn(
        `Unexpected error: command: ${theCommand} (${this.match})`,
      );
  }
  ...
  list() {
    ...
    /** @type {EffectData[]} */
    let sortedEffects = [];
    let effectIds = Object.values(
      CONFIG.statusEffects.map((it) => i18n(it.id)),
    );
    ...
    for (const id of effectIds) {
      ...
      if (effect) {
        effect.posture =
          !!GURPS.StatusEffect.getAllPostures()[id] ||
          id == GURPS.StatusEffectStanding;
        sortedEffects.push(effect);
      } else if (id == GURPS.StatusEffectStanding)
        sortedEffects.push({
          id: id,
          label: GURPS.StatusEffectStandingLabel,
          posture: true,
        });
    }
  ...
  getSelfTokens() {
    ...
    let msg =
      list && list.length === 0
        ? i18n('GURPS.chatNoOwnedTokenFound')
        : i18n('GURPS.chatMultipleOwnedFound');
    ui.notifications.warn(msg);
    return null;
  }
  ...
  async set(tokens, effect) {
    for (const token of tokens) {
      if (!this.isEffectActive(token, effect))
        await this.toggleTokenEffect(token, effect, 'GURPS.chatToggling');
    }
  }
  ...
  async clear(tokens) {
    for (const token of tokens)
      for (const actorEffect of token.actor?.effects || [])
        await this.toggleTokenEffect(
          token,
          this.getStatusEffect(actorEffect),
          'GURPS.chatClearing',
        );
  }
