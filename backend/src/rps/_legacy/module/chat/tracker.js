...
export default class TrackerChatProcessor extends ChatProcessor {
  ...
  matches(line) {
    this.match = line.match(
      /^\/(tracker|tr|rt|resource)([0123])?( *\(([^\)]+)\))? +([+-=] *\d+)?(reset)?(.*)/i,
    );
    return !!this.match;
  }
  ...
  async process(line) {
    ...
    if (!theTracker) {
      ui.notifications.warn(
        `${i18n('GURPS.chatNoResourceTracker', 'No Resource Tracker matched')} 'tr${m[2]}'`,
      );
      return false;
    }
    ...
    if (!!m[6]) {
      ...
      let value = !!theTracker.isDamageTracker
        ? theTracker.min
        : theTracker.max;
      //if (!!theTracker.isDamageTracker) max = 0
      await actor.update({
        ['data.additionalresources.tracker.' + theTrackerKey + '.value']: value,
      });
      this.prnt(
    ...
    if (!m[5]) {
      ui.notifications.warn(
        `${i18n('GURPS.chatUnrecognizedFormat', 'Unrecognized format')} '${line}'`,
      );
      return false;
    }
    ...
    if (isNaN(delta)) {
      ...
      if (isNaN(value)) {
        ...
        value =
          theTracker.isMaximumEnforced && value > theTracker.max
            ? theTracker.max
            : value;
        value =
          theTracker.isMinimumEnforced && value < theTracker.min
            ? theTracker.min
            : value;
        await actor.update({
          ['data.additionalresources.tracker.' + theTrackerKey + '.value']:
            value,
        });
        this.prnt(
          `${i18n('GURPS.chatResourceTracker')}${display} set to ${value}`,
        );
        return true;
      }
    } else {
      ...
      this.prnt(
        `${i18n('GURPS.chatResourceTracker')}${display} ${m[5]} = ${value}`,
      );
      return true;
    }
  }
