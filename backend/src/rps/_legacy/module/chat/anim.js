...
export class AnimChatProcessor extends ChatProcessor {
  ...
  async drawEffect(effect, fromToken, toTokensArray) {
    ...
    for (const to of toTokensArray)
      if (effect.centered)
        used = [
          ...used,
          ...(await this.drawSpecialToward(effect, to, fromToken)),
        ];
      else
        used = [
          ...used,
          ...(await this.drawSpecialToward(effect, fromToken, to)),
        ];
    return used;
  }
  ...
  async drawSpecialToward(effect, tok1, tok2) {
    ...
    let rotation =
      effect.noRotate || (effect.centered && !effect.move)
        ? 0
        : Math.atan2(deltaY, deltaX);
    let distance =
      effect.centered && !effect.move
        ? 0
    ...
    for (let i = 0; i < effect.count; i++) {
      ...
      if (effectData.centered) {
        ...
        effectData.scale = {
          x: s,
          y: Math.random() < 0.5 ? -effectData.scale.y : effectData.scale.y,
        }; // randomly flip vert orientation
      }
  ...
  async process(line) {
    ...
    if (m.file[0] == '/') files = [m.file.substr(1)];
      ...
      let pat = new RegExp(
        m.file.split('*').join('.*?').replace(/\//g, '\\/'),
        'i',
      );
      files = ANIM_LIBRARY.filter((e) => e.match(pat));
      if (files.length == 0)
        return this.errorExit(`Unable to find animation for '${pat}'`);
    }
    ...
    if (!!m.stretch) {
      ...
      if (centered && !move)
        this.priv('Stretch option only valid on moving animations');
    }

    let srcToken = canvas.tokens.placeables.find(
      (e) => e.actor == GURPS.LastActor,
    );
    if (!srcToken) srcToken = canvas.tokens.controlled[0];
    let destTokens = Array.from(game.user.targets);
    if (destTokens.length == 0 && game.user.isGM)
      destTokens = canvas.tokens.controlled;
