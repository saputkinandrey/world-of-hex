...
import StatusChatProcessor from './status.js';
import SlamChatProcessor from './slam.js';
import TrackerChatProcessor from './tracker.js';
import { Migration } from '../../../lib/migration.js';
import { AnimChatProcessor } from './anim.js';
...
class SoundChatProcessor extends ChatProcessor {
  ...
  async process(line) {
    ...
    if (this.match.groups.wait)
      await wait(parseFloat(this.match.groups.wait.substr(1)) * 1000);
    let data = {
...
class QuickDamageChatProcessor extends ChatProcessor {
  ...
  matches(line) {
    ...
    if (!!this.match) {
      ...
      return (
        this.action?.action?.type == 'damage' ||
        this.action?.action?.type == 'roll'
      );
    }
    return false;
  }
  ...
  process(line) {
    ...
    for (const token of destTokens) {
      let owners = game.users.contents.filter(
        (u) =>
          token.actor.getUserLevel(u) >= CONST.DOCUMENT_PERMISSION_LEVELS.OWNER,
      );
      for (const user of owners) if (!user.isGM) users.push(user);
    }
    if (users.length == 0) return false;
    this.registry.processLine(
      '/w [' + users.map((u) => u.name).join(',') + '] ' + this.match[1],
    );
  }
}
...
class ReimportChatProcessor extends ChatProcessor {
  ...
  process(line) {
    ...
    for (const name of actornames) {
      let actor = allPlayerActors.find((a) =>
        a.name.match(makeRegexPattern(name, false)),
      );
...
class ChatExecuteChatProcessor extends ChatProcessor {
  ...
  process(line) {
    ...
    GURPS.chatargs = args;
    let m = Object.values(game.macros.contents).filter((m) =>
      m.name.startsWith(args[0]),
    );
    if (m.length > 0) {
      this.send();
      GURPS.chatreturn = m[0].execute(args) ?? GURPS.chatreturn; // if Advanced macros is loaded, take advantage of the return value
    } else
      this.priv(`${i18n('GURPS.chatUnableToFindMacro')} '${line.substr(2)}'`);
  ...
  async process(line) {
    ...
    if (m[6]?.trim().toLowerCase() == '@target') {
      ...
      targets.map((tid) => {
        ...
        this.priv(
          `${i18n_f('GURPS.chatSentTo', { cmd: line, name: ta.name })}`,
        );
      });
    ...
    if (!!m[5]) {
      ...
      if (isNaN(delta))
        ui.notifications.warn(
          `${i18n('GURPS.chatUnrecognizedFormat')} '${line}'`,
        );
      else {
        let mtxt = '';
        if (delta > max) {
        ...
        this.prnt(
          `${actor.displayname} ${i18n('GURPS.chatSetTo')} ${delta} ${attr}${mtxt}`,
        );
      }
    } else if (!!m[2] || !!m[3]) {
      let mtxt = '';
...
class SelectChatProcessor extends ChatProcessor {
  ...
  process(line) {
    ...
    if (!!m[2]) {
      ...
      if (a.length == 0)
        ui.notifications.warn(
          i18n('GURPS.chatNoActorFound') + " '" + m[3] + "'",
        );
...
class RollChatProcessor extends ChatProcessor {
  ...
  async process(line) {
    ...
    if (!!action.action) {
      ...
      ui.notifications.warn(
        `${i18n('GURPS.chatUnrecognizedFormat')} '[${m[2]}]'`,
      );
    return false;
  }
}
...
class UsesChatProcessor extends ChatProcessor {
  help() {
    return '/uses &lt;formula&gt; &lt;armor-builder name&gt;';
  }
  matches(line) {
  ...
  async process(line) {
    ...
    if (!actor)
      ...
      if (!!pattern) [eqt, key] = actor.findEquipmentByName(pattern, !!m2[1]);
        ...
        // is armor-builder, apply to that armor-builder.
        if (!!k) {
          key = k;
          eqt = getProperty(actor, key);
          // if its not armor-builder, ignore.
          if (eqt.count == null) eqt = null;
        }
      }
      if (!eqt)
        ui.notifications.warn(
          i18n('GURPS.chatNoEquipmentMatched') + " '" + pattern + "'",
        );
      else {
        if (!m[1] && !m[2]) {
          // no +-= or reset
          ...
          if (!!m[2]) {
            this.prnt(
              `${eqt.name} ${i18n('GURPS.chatUsesReset')} (${eqt.maxuses})`,
            );
            ...
            if (isNaN(q))
              ui.notifications.warn(
                eqt.name + ' ' + i18n('GURPS.chatUsesIsNaN'),
              );
            else if (q < 0)
              ui.notifications.warn(
                eqt.name + ' ' + i18n('GURPS.chatDoesNotHaveEnough'),
              );
            else if (!isNaN(max) && max > 0 && q > max)
              ui.notifications.warn(
                `${i18n('GURPS.chatExceededMaxUses')} (${max}) ${i18n('GURPS.for')} ` +
                  eqt.name,
              );
            else {
...
class QtyChatProcessor extends ChatProcessor {
  help() {
    return '/qty &lt;formula&gt; &lt;armor-builder name&gt;';
  }
  matches(line) {
    ...
    if (!actor)
      ...
      if (!!pattern) [eqt, key] = actor.findEquipmentByName(pattern, !!m2[1]);
        ...
        if (!!k) {
          key = k;
          eqt = getProperty(actor, key);
          // if its not armor-builder, try to find armor-builder with that name
          if (eqt.count == null)
            [eqt, key] = actor.findEquipmentByName(
              (pattern = eqt.name),
              !!m2[1],
            );
        }
      ...
      if (!eqt)
        ...
        if (isNaN(delta)) {
          ...
          if (isNaN(delta))
            ui.notifications.warn(
              `${i18n('GURPS.chatUnrecognizedFormat')} '${m[1]}'`,
            );
          else {
            answer = true;
...
class LightChatProcessor extends ChatProcessor {
  ...
  async process(line) {
    ...
    if (line.match(/^\/(light|li) *$/)) {
      this.priv(
        'Possible animations: ' +
          Object.keys(CONFIG.Canvas.lightAnimations).join(', '),
      );
      return;
    }
    ...
    if (!!type) {
      ...
      let m = Object.keys(CONFIG.Canvas.lightAnimations).find((k) =>
        k.match(pat),
      );
      if (!m) {
        ui.notifications.warn(
          "Unknown light animation '" +
            type +
            "'.  Expected: " +
            Object.keys(CONFIG.Canvas.lightAnimations).join(', '),
        );
    ...
    if (this.match.groups.off) {
      ...
      if (this.match.groups.colorint)
        data.light.alpha = parseFloat(this.match.groups.colorint);
...
class ShowChatProcessor extends ChatProcessor {
  ...
  async process(line) {
    ...
    for (const orig of args) {
      this.priv('<hr>');
      if (orig.toLowerCase() == 'move')
        this.priv('<b>Basic Move / Current Move</b>');
      if (orig.toLowerCase() == 'speed') this.priv('<b>Basic Speed</b>');
      if (orig.toLowerCase() == 'hp')
        this.priv('<b>Hit Points: Current / Max</b>');
      if (orig.toLowerCase() == 'fp')
        this.priv('<b>Fatigue Points: Current / Max</b>');
      let output = [];
      for (const token of canvas.tokens.placeables) {
        let arg = orig;
        let actor = token.actor;
        let skip =
          (npc && actor.hasPlayerOwner) || (pc && !actor.hasPlayerOwner);
        if (!skip) {
...
class DevChatProcessor extends ChatProcessor {
  ...
  async process(line) {
    ...
    switch (m[1]) {
      ...
      case 'reset': {
        GURPS.executeOTF(
          '/sel *\\\\/sr [/hp reset]\\\\/sr [/fp reset]\\\\/sr [/st clear]',
        );
        break;
      }
    }
...
class ManeuverChatProcessor extends ChatProcessor {
  ...
  async process(line) {
    ...
    let m = Object.values(Maneuvers.getAll()).find((e) =>
      i18n(e.data.label).toLowerCase().match(r),
    );
    if (!GURPS.LastActor) {
      ui.notifications.warn(i18n('GURPS.chatYouMustHaveACharacterSelected'));
      return false;
    ...
    if (!m) {
      ui.notifications.warn(
        i18n('GURPS.chatUnableToFindManeuver') + " '" + this.match[2] + "'",
      );
      return false;
    }
...
class RepeatChatProcessor extends ChatProcessor {
  ...
  async repeatLoop(actor, anim, delay) {
    ...
    if (!t) {
      ui.notifications.warn(
        "/repeat only works on 'linked' actors, " + actor.name,
      );
      return false;
    }
