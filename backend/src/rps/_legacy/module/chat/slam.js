...
export default class SlamChatProcessor extends ChatProcessor {
  ...
  async process(line) {
    ...
    let attacker =
      actor.token || canvas.tokens.placeables.find((it) => it.actor === actor);

    if (targets.length === 1)
      SlamCalculatorForm.process(attacker, [...targets][0]);
    else if (targets.length > 1)
      selectTarget().then((target) =>
        SlamCalculatorForm.process(attacker, target),
      );
...
class SlamCalculatorForm extends FormApplication {
  ...
  constructor(attacker, target, options = {}) {
    ...
    this._attackerSpeed = !!attacker
      ? parseInt(attacker.actor.system.basicmove.value)
      : 5;
