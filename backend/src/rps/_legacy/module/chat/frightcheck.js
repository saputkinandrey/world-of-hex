...
export class FrightCheckChatProcessor extends ChatProcessor {
  ...
  async process(line) {
    ...
    let tblname =
      game.settings.get(
        ...
        Settings.SETTING_FRIGHT_CHECK_TABLE,
      ) || 'Fright Check';

    // TODO reskin frightcheck UI
    renderTemplate('systems/gurps/templates/frightcheck-macro.html', {
      tblname: tblname,
    }).then((dialogTemplate) =>
      new Dialog(
        {
  ...
  async rollFrightCheckCallback(actor, html) {
    ...
    let WILLVar = parseInt(
      actor.system.frightcheck || actor.system.attributes.WILL.value,
      10,
    );
    ...
    await ChatMessage.create({
      ...
      if (failure) {
        ...
        table
          .draw({ roll: tableRoll })
          .then(() => GURPS.setLastTargetedRoll({ margin: margin }, actor)); // don't evaluate before passing
      }
    });
  }
  ...
  _getMod(html, id) {
    ...
    if (mod.type === 'select' || mod.type === 'select-one') {
      ...
      return {
        ...
        desc: mod.options[mod.selectedIndex].text,
      };
