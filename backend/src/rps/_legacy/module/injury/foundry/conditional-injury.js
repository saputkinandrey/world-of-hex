...
export default class GURPSConditionalInjury {
  constructor() {
    ...
    Handlebars.registerHelper(
      'ciCurrentGrossEffects',
      self.currentGrossEffects,
    );
  }
  ...
  isInUse = () =>
    game.settings.get(
      ...
      Settings.SETTING_USE_CONDITIONAL_INJURY,
    );
  ...
  severitiesTooltip = (opt) => {
    const data = CI.conditionalEffectsTable.map((row) => ({
      severity: row.severity,
      label: row.grossEffects,
    }));
    data.forEach((row) => {
