...
export function makeRegexPatternFrom(text, end = true, start = true) {
  ...
  let pattern = text
    ...
    .join('.*?')
    .replace(/\(/g, '\\(')
    .replace(/\)/g, '\\)'); // Make string into a RegEx pattern
  pattern = pattern
    .replace(/\[/g, '\\[')
    .replace(/\]/g, '\\]')
    .replace(/\+/g, '\\+');
      ...
      setTimeout(
        () =>
          Dialog.confirm({
            ...
            content: i18n_f('GURPS.chatWantsToExecute', {
            ...
            yes: (y) => {
              ...
              GURPS.executeOTF(resp.command).then((p) =>
                GURPS.SetLastActor(old),
              );
            },
          }),
        50,
