...
import { GurpsActor } from '../_legacy/module/actor/actor.js';
import {
  Skill,
  Advantage,
  Ranged,
  ...
  Encumbrance,
  Equipment,
} from '../_legacy/module/actor/actor-components.js';
...
Hooks.once('init', async function () {
  game.settings.registerMenu(
    ...
    settings.SETTING_MOOK_DEFAULT_EDITOR,
    {
...
export class NpcInput extends FormApplication {
  constructor(actor, options = {}) {
    super(options);
    let m = game.settings.get(
      settings.SYSTEM_NAME,
      settings.SETTING_MOOK_DEFAULT,
    );
    ...
    let d = new Dialog(
      {
        ...
        content: await renderTemplate(
          'systems/gurps/templates/import-stat-block.html',
          { block: b },
        ),
        buttons: {
          import: {
  ...
  async populate(a) {
    ...
    for (const [key, value] of Object.entries(
      HitLocations.hitlocationDictionary.humanoid,
    )) {
    ...
    ts.createdon = dt;
    if (!!m.sm)
      ts.sizemod = m.sm[0] == '-' ? m.sm : m.sm[0] == '+' ? '' : '+' + m.sm;
  ...
  gatherAttackLines(attblk, currentline, oldformat) {
    ...
    while (
      !!nextline &&
      (nextline.match(/[Ss]hots.*\(\d+\)/) || !nextline.match(/\(\d+\)/))
    ) {
  ...
  parseEquipment() {
    ...
    let line = this.nextline(); // Skip armor-builder line
    line = this.nextline();
    while (!!line) {
      var m;
      let cost = '?';
  ...
  parseS(start, end, path) {
    ...
    while (!!s) {
      ...
      l = this.cleanLine(l).replace(
        / ?\([a-zA-Z]+\) [a-zA-Z]+([-+][0-9]+)? \[ *-?\d+\ *] */g,
        '',
      );
  ...
  parseTraits() {
    ...
    if (!trblk)
      return this.appendToNotes(
        ERR + " Looking for Traits block, unable to find 'Skills'",
      ); // Basically always need "Skills" or "Spells"
    let traits = '';
  ...
  parseAttacks(oldformat = false) {
    ...
    if (oldformat) {
      ...
      if (!attblk) {
        if (this.peek('Skills:')) attblk = this.nextToken('Skills:', 'junk');
          ...
          this.mook.melee = COMMENT_CHAR + 'No attacks found'; // If Weapons aren't listedt later, show error.
      }
    }
    if (!attblk) return;
    ...
    while (!!line) {
      ...
      if (!name) {
        ...
        [attblk, line, nextline] = this.gatherAttackLines(
          attblk,
          nextline,
          oldformat,
        ); // collect up any more lines.
        continue;
      }
      name = name.trim();
  ...
  cleanLine(line) {
    ...
    if (pat.includes(line[line.length - 1]))
