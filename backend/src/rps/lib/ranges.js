...
import RepeatingSequenceConverter from '../_legacy/module/utilities/repeating-sequence.js';
...
export default class GURPSRange {
  ...
  yardsToSpeedRangePenalty(yards) {
    let currentValue = game.settings.get(
      Settings.SYSTEM_NAME,
      Settings.SETTING_RANGE_STRATEGY,
    );
  ...
  _buildModifiers() {
    /** @type {import('../_legacy/module/modifier-bucket/bucket-app.js').Modifier[]} */
    let m = [];
    this.ranges.forEach((band) => {
      if (band.penalty != 0)
        GURPS.ModifierBucket.addModifier(band.penalty, band.moddesc, m);
    });
