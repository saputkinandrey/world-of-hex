...
import { HitLocation } from '../_legacy/module/hitlocation/hitlocation.js';
...
export const COSTS_REGEX =
  /.*\* ?(?<verb>(cost|per|costs))? (?<cost>\d+) ?(?<type>[ \w\(\)]+)/i;
...
export function parseForRollOrDamage(str, overridetxt) {
  ...
  if (!!a) {
    ...
    let [actualType, extDamageType, hitLocation] =
      _parseOtherForTypeModiferAndLocation(other);
  ...
  if (!!a) {
    ...
    const other = !!a.groups.other ? a.groups.other.trim() : '';
    const [actualType, extDamageType, hitLocation] =
      _parseOtherForTypeModiferAndLocation(other);
...
function _parseOtherForTypeModiferAndLocation(other) {
  ...
  if (!!dmgTypeMatch)
    if (dmgTypeMatch[2].includes('@')) {
      ...
      hitLocation = !!loc.trim()
        ? HitLocation.translate(loc.trim())
        : undefined;
...
function _getFormulaComponents(groups) {
  ...
  if (!!multiplier && 'Xx×'.includes(multiplier[0]))
    multiplier = '*' + multiplier.substr(1); // Must convert to '*' for Foundry.
...
export function parselink(str, htmldesc, clrdmods = false) {
  ...
  // if [@margin] [then]
  m = str.match(
    /^@(margin|isCritSuccess|IsCritFailure) *([=<>]+ *[+-]?[\d\.]+)?$/i,
  );
  if (!!m) {
    let action = {
      type: 'iftest',
  ...
  if (!!m) {
    ...
    if (!!path) {
      let opt =
        m[4].trim().match(/([^\?]*)(\? *"([^"]*)")?( *[,:] *"([^"]*)")?/) || []; // desc (searching for true/false options)
      let desc = opt[1].trim();
      let spantext = attr;
      ...
      if (!!m[5]) {
        ...
        if (
          def.action?.type == 'skill-spell' ||
          def.action?.type == 'attribute'
        )
          spantext += ' | ' + def.action.spantext;
        else def = {};
      }
  ...
  if (parse != str) {
    ...
    if (!!name) {
      ...
      if (!!matches) {
        ...
        if (
          attribute.action?.type === 'attribute' ||
          attribute.action?.type === 'mapped'
        ) {
