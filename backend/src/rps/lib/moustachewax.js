...
import * as HitLocations from '../_legacy/module/hitlocation/hitlocation.js';
import {
  isArray,
  isEmpty,
  getComparison,
...
import Maneuvers from '../_legacy/module/actor/maneuver.js';
import { parseDecimalNumber } from './parse-decimal-number/parse-decimal-number.js';
import { MoveModes } from '../_legacy/module/actor/actor.js';
import { multiplyDice } from '../_legacy/module/utilities/damage-utils.js';
import { gurpslink } from '../_legacy/module/utilities/gurpslink.js';
...
export default function () {
  ...
  Handlebars.registerHelper(
    'chooseplural',
    function (quantity, single, plural) {
  ...
  */
  ...
  Handlebars.registerHelper(
    'printIfNe',
    function (value, compareTo, format, _default = '') {
  ...
  Handlebars.registerHelper(
    'gurpslink',
    function (str, root, clrdmods = false) {
  ...
  const flatlist = function (
    ...
    for (let key in context) {
      ...
      if (display) {
        ...
        newItem['hasCollapsed'] =
          !!item?.collapsed && Object.values(item?.collapsed).length > 0;
        newItem['hasContains'] =
          !!item?.contains && Object.values(item?.contains).length > 0;
        newItem['isCollapsed'] = isCollapsed;

        data[newKey] = newItem;

        if (newItem['hasContains'])
          flatlist(
            item.contains,
            level + 1,
            newKey + '.contains.',
  ...
  Handlebars.registerHelper('woundModifierText', function (calc) {
    let add = calc.additionalWoundModifier
      ? ` + ${calc.additionalWoundModifier} add`
      : '';
    let vul = calc.isVulnerable
      ? ` × ${calc.vulnerabilityMultiple} (Vuln.)`
      : '';
  ...
  Handlebars.registerHelper('showTheMath', function () {
    return game.settings.get(
      settings.SYSTEM_NAME,
      settings.SETTING_SHOW_THE_MATH,
    )
      ? 'checked'
      : '';
  });
  ...
  Handlebars.registerHelper('pdflinkext', function (obj) {
    ...
    return txt
      ...
      .map((/** @type {string} */ l) => {
        if (!!obj.externallink)
          return `<a href="${obj.externallink}">*Link</a>`;
  ...
  Handlebars.registerHelper('displayDecimal', function (num, options) {
    if (num != null) {
      ...
      if (options.hash?.forceSign && num.toString()[0] !== '+')
        return `+${num}`;
      return num.toString();
    } else return ''; // null or undefined
  });
  ...
  Handlebars.registerHelper('trackerIndex', function (data, trackerName) {
    if (!!data && !!data.additionalresources?.tracker) {
      ...
      let found = Object.keys(tracker).find(
        (it) => tracker[it].name === trackerName,
      );
      if (!!found) {
        return found;
      }
  ...
  Handlebars.registerHelper('tracker', function (data, trackerName) {
    if (!!data && !!data.additionalresources?.tracker) {
      ...
      let tracker = Object.values(data.additionalresources?.tracker).find(
        (it) => it.name === trackerName,
      );
      if (!!tracker) {
        return tracker;
      }
  ...
  Handlebars.registerHelper('isUserCreated', function (obj) {
    return (
      game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_SHOW_USER_CREATED,
      ) && !!obj.save
    );
  });
  ...
  Handlebars.registerHelper('isFoundryItem', function (obj) {
    return (
      game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_SHOW_FOUNDRY_CREATED,
      ) && !!obj.itemid
    );
  });
  ...
  Handlebars.registerHelper('ignoreImportQty', function (obj) {
    return (
      game.settings.get(
        settings.SYSTEM_NAME,
        settings.SETTING_ignoreImportQty,
      ) && !!obj.ignoreImportQty
    );
  });
  ...
  Handlebars.registerHelper(
    'collapsible-content',
    function (id, data, group, options) {
      let title = data[0];
      let type =
...
`;
  ...
  Handlebars.registerHelper('damageTerm', function (calc, options) {
    ...
    let damageType =
      calc.damageType !== 'dmg' &&
      calc.damageType !== 'injury' &&
      calc.damageType !== 'none'
        ? calc.damageType
        : '';
  ...
  const templates = [
    ...
    'systems/gurps/templates/actor/sections/armor-builder.hbs',
