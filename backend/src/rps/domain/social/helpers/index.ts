...
export function getSocialBalance(roleA: SocialRelationEntity, roleB: SocialRelationEntity): number {
    const obedience = roleB.dutiesBeforeLeader.find((d) => d.tag === SocialDutyTag.OBEY_ORDERS)?.value ?? 0;
    const command = roleA.rightsOnSubordinates.find((r) => r.tag === SocialRightTag.COMMAND)?.value ?? 0;
...
export function performRespectRitual(subject: CharacterEntity, leader: CharacterEntity) {
