...
export class PlayerService {
    ...
    ownAShip(player: PlayerDocument, ship: Ship) {
        ...
        return this.playerRepository
            .findOwnerByShipId(ship._id)
            .then((owner) => {
                if (owner && owner._id.toString() !== player._id.toString()) {
                    throw new Error(
                        `Ship ${ship._id} already owned by player ${owner._id}`,
                    );
                }
    ...
    async unownShip(player: PlayerDocument, shipId: string) {
        ...
        player.ownedShips = player.ownedShips.filter(
            (owned) => owned._id !== shipId,
        );
        if (player.ownedShips.length === beforeCount) {
            throw new Error(
                `Player ${player._id} does not own ship with id ${shipId}`,
            );
        }
        return player.save();
    }
