...
async function upsertCategories(rows: RowCategorySummary[]) {
    ...
    for (const r of rows) {
        ...
        const totalOccurrences =
            r.total_occurrences != null
                ? toInt(r.total_occurrences)
                : undefined;
        ...
        try {
            ...
            console.warn(
                `Skipping category row "${rawCat}": ${(err as Error).message}`,
            );
            fail++;
            continue;
        }
...
async function main() {
    ...
    console.log(
        `Loaded CSV rows: keys=${keyRows.length}, categories=${catRows.length}`,
    );
    ...
    if (RECOMPUTE && !DRY_RUN) {
        ...
        console.log(
            `Recomputed totalOccurrences from keys for ${affected} categories`,
        );
    }
